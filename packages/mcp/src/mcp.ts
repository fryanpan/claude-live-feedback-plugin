...
const server = new Server(
  ...
  },
  {
    ...
    instructions: [
      ...
      '',
      'CREATING A REVIEW DOC: pass sourceUrl in the POST /api/docs body. The server',
      'auto-attaches the file (reads it, parses it, seeds the live doc) before any',
      'reviewer arrives — so when the reviewer opens /review/<docId>, they see the',
      'file content, not an empty editor. Without sourceUrl, the doc is empty and',
      'the markdown-app client seeds a welcome placeholder when the reviewer opens',
      "the URL, which then makes seed_doc fail with 409 non-empty (and you'll have",
      'no clean way to populate from a file afterward).',
      '',
      'EMPTY-DOC DIAGNOSIS: if a reviewer reports an empty browser display, do',
      'NOT reach for seed_doc as the fix. Call get_doc first to see actual server',
      'state. If get_doc returns the welcome placeholder (text starting with',
      'Welcome. Select any text...), the doc was created without sourceUrl —',
      'recreate it with sourceUrl set, or call reparse_from_disk after',
      'attach_file to wipe and reload. If get_doc returns real content, ask the',
      'reviewer to refresh.',
    ].join(' '),
  },
);
