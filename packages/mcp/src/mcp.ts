 ...
 */
function toolContext(): DocsToolContext & TaskToolContext {
  return {
    http,
    ok,
    err,
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    // Documents answer from tools/docs.ts, board rows from tools/tasks.ts. A
    // domain handler returns `undefined` for a name that is not its own, so
    // the families chain the way the server's route files do.
