...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
      ...
      inputSchema: {
        ...
        properties: {
          ...
          needs: {
            ...
            description:
              "Only meaningful when assignee is a human. 'decision' makes it a decision task (answer_decision records the verbatim answer).",
          },
          goal: {
            type: 'string',
    ...
    },
    {
      name: 'set_goal_list',
      description:
        'Replace a workspace\'s ORDERED goal list — the board\'s sections; order is priority, so reordering goals is the priority gesture (it fires workspace.goals_changed, never a re-triage). Each goal: {id, title, dueAt?, subgoals?: [{id, title, dueAt?}]} — one subgoal level max. "chores" is reserved (always rendered last, never in the list). DESTRUCTIVE EDGE: open tasks whose goal id disappears move to the bottom of Chores — the result reports movedToChores so you can re-place each with set_task_goal; do that rather than leaving them piled.',
      inputSchema: {
        type: 'object',
        properties: {
          workspaceId: { type: 'string' },
    ...
    switch (name) {
      ...
      case 'create_workspace': {
        ...
        const res = (await http('POST', '/api/workspaces', {
          name: wsName,
          ...(goal !== undefined ? { goal } : {}),
        })) as { workspace: { id: string; name: string; goal: string } };
      ...
      case 'set_goal_list': {
        const { workspaceId, goals } = a as { workspaceId: string; goals: unknown[] };
        const res = (await http('PUT', `/api/workspaces/${encodeURIComponent(workspaceId)}/goals`, {
          goals,
          author: AUTHOR,
        })) as { changed: boolean; movedToChores: string[] };
      ...
      case 'set_workspace_goal': {
        ...
        const res = (await http('PUT', `/api/workspaces/${encodeURIComponent(workspaceId)}/goal`, {
          goal,
          author: AUTHOR,
        })) as { changed: boolean; retriage: { requested: boolean; taskIds: string[] } };
        return ok({ workspaceId, changed: res.changed, retriage: res.retriage });
      }
      case 'answer_decision': {
        const { taskId, text } = a as { taskId: string; text: string };
