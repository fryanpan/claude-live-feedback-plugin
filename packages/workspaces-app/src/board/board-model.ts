 ...
 *
 * This is the base of the board's model layer: `board-review-model.ts` and
 * `board-presence-model.ts` both import from here, and nothing here imports
 * either of them.
 */
import {
 ...
 * notice here and the uptime banner in `board-presence-model.ts` are the two
 ...
 * owner's.
 */
export type BoardOwnerKind = 'person' | 'agent' | 'unknown';
...
export type BoardDecisionOption = DecisionOption;
...
export interface BoardInfoRequest {
   ...
   *  what makes the decision `waiting` — see `BoardTask.decisionState`. */
  threadId?: string;
}
...
export interface BoardTask {
  ...
  /** The agent's own one-liners on the row, newest first (see `BoardNote`).
 ...
 * predicate is spelled again here because the board reads the PROJECTION, which
 * is a wire shape rather than a `TaskReviewItem`.
 */
export function heldReviewItems(task: BoardTask): BoardReviewItem[] {
...
export interface BoardGoal {
   ...
   * projected the same way `BoardTask.links` is (`task-projection.ts`). The
...
export function doneAt(task: BoardTask): number {
 ...
 */
export function ownerKind(task: BoardTask): BoardOwnerKind {
 ...
 */
export function ownedByPerson(task: BoardTask): boolean {
 ...
 * clock moves.
 */
export function isTaskArchived(task: BoardTask): boolean {
...
export function archivedTasks(tasks: BoardTask[]): BoardTask[] {
...
export function isGoalArchived(goal: BoardGoal): boolean {
 ...
 */
export function archivedGoals(goals: BoardGoal[]): BoardGoal[] {
  return goals.filter(isGoalArchived).sort((a, b) => (b.archivedAt ?? 0) - (a.archivedAt ?? 0));
}

export function taskVisible(task: BoardTask, f: BoardFilters): boolean {
...
export interface BoardSection {
  ...
  title: string;
  dueAt?: number;
  /** The band's own status, carried from the goal row (see `BoardGoal`).
   *  Absent on Backlog — a bucket, not a goal — and on undecorated bands. */
  status?: TaskStatus;
  doneAt?: number;
  doneBy?: BoardActor;
  /** The goal row's owner, carried the same way (see `BoardGoal`). */
  assignee?: string;
  ownerKind?: BoardOwnerKind;
  /** The goal's description and its discussion, carried the same way again —
   *  see `BoardGoal` for what each one is and why the address rides even
   *  when the text does not. Absent on Backlog: a bucket has no body. */
  bodyDocId?: string;
  body?: string;
  bodyTruncated?: boolean;
  commentCount?: number;
  /** The docs this goal ties to, carried the same way again — see `BoardGoal`.
...
function carriedOf(g: BoardGoal) {
  return {
 ...
 */
export function goalSection(goals: BoardGoal[], goalId: string): BoardSection | null {
...
export function scheduleRules(tasks: readonly BoardTask[]): Map<string, BoardTask> {
 ...
 */
export function boardBlockers(tasks: readonly BoardTask[]): Map<string, string[]> {
...
export function goalBandIds(goals: BoardGoal[]): Set<string> {
 ...
 * call site.
 */
export function boardCalibration(goals: BoardGoal[], tasks: BoardTask[]): EffortCalibration {
...
export function boardEffort(goals: BoardGoal[], tasks: BoardTask[], now: number): BoardEffort {
  const known = goalBandIds(goals);
  ...
  // arithmetic.
  const bandOf = (task: BoardTask): string => bandOfGoal(known, task.goal);
 ...
 */
export function goalRank(goals: BoardGoal[]): (goalId: string) => number {
 ...
 */
export function goalLabel(goals: BoardGoal[], goalId: string): string {
  for (const g of goals) {
 ...
 */
export function unplacedNotice(tasks: BoardTask[], now: number): UnplacedNotice | null {
  let count = 0;
  let oldest: BoardTask | null = null;
 ...
 */
export function ownerMarkKind(task: BoardTask, owner: string): OwnerMarkKind {
