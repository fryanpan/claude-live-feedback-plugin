 ...
 *
 * `BoardDetailDeps` is long — longer than any other region's — and that is the
 ...
 * discussion before it is called stale. `bootBoard` owns both — it is the
...
/** Everything the panel needs from `bootBoard`, and nothing else. */
export interface BoardDetailDeps {
  ...
  document: Document;
  /** The shareable address of one task, and of one band. `bootBoard` owns the
...
/** What `bootBoard` keeps: the render, and the one thing that aims it. */
export interface BoardDetailPanel {
  renderDetail(): void;
  /** Aim the next render's title at a row, so a task filed empty opens with
   *  the cursor in its name — `createBoardActions`'s `focusTitle`. */
  setFocusTitle(taskId: string): void;
}

export function createBoardDetailPanel(deps: BoardDetailDeps): BoardDetailPanel {
  const {
    state,
    user,
    workspaceId,
  ...
  const openBlockersOf = (task: BoardTask): { taskId: string; title: string }[] =>
  ...
  function syncReadTracker(taskId: string | null, bodyDocId?: string): void {
    ...
    const host = document.getElementById('board-detail');
    if (!host) return;
    readTracker = {
      taskId,
      stop: startReadingTracker({
        docId: bodyDocId,
        user,
        // `.board-detail-panel` is the element with `overflow: auto`, so it is
        ...
        scrollEl: () => host.querySelector<HTMLElement>('.board-detail-panel'),
  ...
  function renderDetail(): void {
    ...
    taskDetailData.value = {
      ...
      handlers: {
        ...
        // paint synchronously: reading `.board-detail-body-slot` off the DOM on
   ...
   */
  async function copyTaskLink(task: BoardTask): Promise<void> {
