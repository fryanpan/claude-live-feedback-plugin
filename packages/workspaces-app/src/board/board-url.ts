 ...
 *
 * Every openable thing on the board — a nav destination, a task or goal panel,
 ...
 * names. `board-app.ts` owns WHEN history is written (its one `syncBoardUrl`
 ...
 */
import { type BoardNav, navFromPath, navPath } from './board-presence-model.ts';
