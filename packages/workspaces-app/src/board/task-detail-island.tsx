 ...
 * walkthrough islands: `renderDetail` in board-app still owns the fetches, the
...
function UndoAnswer(props: { undo: () => Promise<boolean> | undefined }) {
  ...
  return (
    <button
      type="button"
      class="board-btn board-detail-undo-answer"
      disabled={busy}
 ...
 *  is the same record. */
function TaskAnsweredNote(props: { task: BoardTask; handlers: DetailHandlers }) {
  const { task, handlers } = props;
  const answer = task.answer;
  return (
    <div class="board-detail-answered">
      <p class="board-detail-answer">
...
function DecideOptions(props: {
  ...
  return (
    <div class="board-decide-options">
      {props.options.map((o) => (
        <button
          key={o.id}
          type="button"
          class="board-decide-option"
          disabled={props.busy}
          onClick={() => props.onPick(o)}
        >
          <span class="board-decide-option-label">{o.label}</span>
          {o.detail && <span class="board-decide-option-detail">{o.detail}</span>}
        </button>
      ))}
    </div>
  );
...
function ReviewCard(props: {
  ...
  // `.board-detail-title`, so the reader would get the same words twice in a
      ...
      {item.answered ? (
        ...
        <Fragment>
          {asking && (
            ...
            <div class="board-decide-question-box">
              <ComposerForm
                className="board-answer-form board-decide-form board-decide-question-form"
              ...
              <button
                type="button"
                class="board-btn board-btn-ghost board-decide-question-cancel"
          ...
          <div class={asking ? 'board-decide-answering hidden' : 'board-decide-answering'}>
            ...
            {canAsk && (
              ...
              <button
                type="button"
                class="board-btn board-btn-ghost board-decide-question-link"
            ...
            {item.source === 'thread' && (
              ...
              <p class="board-decide-question-note">
 ...
 */
function HeldReviewNote(props: { task: BoardTask; handlers: DetailHandlers; now: number }) {
  const { task, handlers, now } = props;
  const held = heldReviewItems(task);
  const [releasing, setReleasing] = useState<string | null>(null);
  if (held.length === 0) return null;
  return (
    <section class="board-decide board-decide--held" aria-label="Review items being revised">
      {held.map((item) => (
        <p class="board-decide-held" key={item.id} data-review-item-id={item.id}>
          <span class="board-decide-held-kicker">Held</span>
          <span class="board-decide-held-ask">{item.review.headline}</span>
          <span class="board-decide-held-why">
            {item.judge?.reason ? `${judgeReasonClause(item.judge.reason)} — ` : ''}
            the agent has been asked to revise it before it reaches your queue.
          </span>
          <span class="board-decide-held-foot">
            {/* Who filed it and how long it has stood, in the same words and
                ...
                2026-08-29). */}
            <span class="board-decide-held-meta">
            ...
            {handlers.onReleaseHeld && (
              <button
                type="button"
                class="board-btn board-decide-held-release"
...
function ReviewRegion(props: {
  ...
  // happened to be first. State rather than a reading of `.board-decide`'s
  ...
  if (queue.length === 0) {
    ...
    return <section class="board-decide board-decide--answered">{answered}</section>;
  }
  ...
  return (
    <section
      class="board-decide"
      data-review-index={String(at)}
      data-review-item-id={item?.id ?? ''}
    >
      {answered}
      <div class="board-decide-head">
        <p class="board-decide-kicker">{kicker}</p>
        ...
        {queue.length > 1 && (
          <div class="board-decide-walk">
            <button
              type="button"
              class="board-btn board-decide-step"
            ...
            <span class="board-decide-count">{`${at + 1} of ${queue.length}`}</span>
            <button
              type="button"
              class="board-btn board-decide-step"
...
interface FeedEntry {
  key: string;
  ts: number;
  kind: 'move' | 'event' | BoardNote['kind'];
  note?: BoardNote;
  build?: () => HTMLLIElement;
}
...
function feedOf(task: BoardTask, events: ActivityEvent[] | undefined): FeedEntry[] {
...
const KIND_LABEL: Record<BoardNote['kind'], string> = {
  turn: 'turn',
  status: 'status',
  denial: 'blocked',
};
...
function NoteRow(props: {
  ...
  return (
    <li
      class={`board-hist-row board-hist-row-${kind}`}
      title={new Date(note.at).toLocaleString()}
      data-hist-key={entry.key}
    >
      <div class="board-note-head">
        <span class="board-note-agent">{note.agent}</span>
        <span class="board-note-kind">{KIND_LABEL[kind]}</span>
        <span class="board-note-age">{ageShort(note.at, now)}</span>
      </div>
      <div ref={bodyRef} class={`board-note-body${folded ? ' is-folded' : ''}`} />
      {long && (
        <button
          type="button"
          class="board-note-more"
 ...
 */
function ActivityTab(props: { task: BoardTask; handlers: DetailHandlers; hidden: boolean }) {
  ...
  useLayoutEffect(() => {
    ...
    const tapWord = (ev: MouseEvent): void => {
      ...
      if (!target?.closest('.board-note-body, .board-hist-words')) return;
  ...
  const rowKey = pill.phrase
    ? pill.at
        ?.closest<HTMLElement>('.board-note-body, .board-hist-words')
        ?.closest<HTMLElement>('li.board-hist-row')?.dataset.histKey
    : undefined;
  const openCard = (): void => {
  ...
  return (
    <div
      class={`board-detail-tabpanel board-detail-tabpanel-activity${hidden ? ' hidden' : ''}`}
      role="tabpanel"
    >
      {!hasFeed && (
        <p class="board-hist-empty">Nothing yet — the first move, edit or note lands here.</p>
      )}
      {hasFeed && (
        <Fragment>
          <h3 class="board-detail-subhead">History</h3>
          <ul ref={historyRef} class="board-detail-transitions">
            {feed.map((entry) => {
      ...
      {canComment && (
        <button
          type="button"
          class={`comment-pill board-hist-pill${rowKey ? '' : ' hidden'}`}
       ...
       */}
      {task.quote && (
        <details class="board-detail-quote-block">
          <summary
            class="board-detail-quote-label"
            title="The words this task came from, kept verbatim."
          >
            Original words
          </summary>
          <blockquote class="board-detail-quote">{task.quote}</blockquote>
        </details>
      )}
...
function RelatedLinkAdd(props: { onAdd: (url: string) => void }) {
  ...
  return (
    <div class="board-related-add">
      <button
        type="button"
        class={open ? 'board-related-add-btn hidden' : 'board-related-add-btn'}
        onClick={(ev) => {
          ev.stopPropagation();
          setOpen(true);
        }}
      ...
      <input
        ref={input}
        type="url"
        class={open ? 'board-related-add-input' : 'board-related-add-input hidden'}
...
function TaskDetailPanel(props: {
  ...
  useLayoutEffect(() => {
    ...
    el.classList.toggle('board-detail-title-placeholder', task.untitled === true);
  ...
  useLayoutEffect(() => {
    ...
    const sync = (): void => {
      ...
      if (h > 0) panel.style.setProperty('--board-detail-head-h', `${Math.round(h)}px`);
    };
    sync();
  ...
  useLayoutEffect(() => {
    ...
    const inQueue =
      panel.querySelector(
        `.board-decide-card[data-review-thread-id="${CSS.escape(focusThreadId)}"]`,
      ) !== null;
    if (inQueue) return;
    const focus = panel.querySelector<HTMLElement>(
      `.board-comment[data-thread-id="${CSS.escape(focusThreadId)}"]`,
    );
  ...
  return (
    <div
      ...
      // inside `.board-detail`'s backdrop by an app that opens and closes it
      ...
      data-tab={tab}
    >
      <div ref={headRef} class="board-detail-head">
      ...
      {hasRelated && (
        <div class="board-related-links-slot">
      ...
      {/* The blocked note (design point 5): this task is a person's own open
          ...
          blocker. */}
      {handlers.blocked && (
        <div class="board-blocked-note">
          <span class="board-decide-k board-blocked-k">Blocking</span>
      ...
      <div ref={tabsRef} class="board-detail-tabs" role="tablist">
        {DETAIL_TABS.map((t) => (
          <button
            key={t.id}
            type="button"
            class={`board-detail-tab board-detail-tab-${t.id}`}
            role="tab"
      ...
      <div
        class={`board-detail-tabpanel board-detail-tabpanel-comments${tab === 'comments' ? '' : ' hidden'}`}
        role="tabpanel"
      >
 ...
 * so this writes nothing Preact believes it owns. `board-detail-open` marks
...
/**
 * Mounts the panel into a wrapper it appends to `host` (`#board-detail`);
 ...
 * shared `#board-detail` and rebuilt it wholesale, now has a container of its
 * own.)
 *
