 ...
 *
 * A factory rather than an inline listener in board-app's `main()` so the
 * handler is testable — board-app runs `main()` on import, which needs a live
 * workspace URL, so nothing in it can be exercised from a test.
 */
import { eventPath, typingInPath } from '../keyboard-target.ts';

export interface BoardShortcutDeps {
  /** Live board state, read at keydown time — pass the state object itself,
   *  never a snapshot of it. Structurally minimal so tests need no BoardTask. */
  state: {
...
export function boardShortcutKeydown(deps: BoardShortcutDeps): (ev: KeyboardEvent) => void {
  const { state } = deps;
  return (ev) => {
    ...
    if (ev.key === 'c') {
      const btn = document.querySelector<HTMLButtonElement>('.board-quick-new');
      if (btn) {
        btn.click();
        ev.preventDefault();
      }
    ...
    const rows = Array.from(document.querySelectorAll<HTMLElement>('.board-task-row'));
    ...
    if (
      anchor < 0 &&
      state.detailTaskId &&
      document.activeElement?.closest('.board-detail-panel')
    ) {
    ...
    if (ev.key === 'j' || ev.key === 'k') {
      ...
      if (ev.key === 'o') {
        ...
        rows[anchor]?.querySelector<HTMLSelectElement>('.board-row-assignee')?.focus();
      } else {
        ...
        rows[anchor]?.querySelector<HTMLSelectElement>('.board-status-select')?.focus();
      }
      ev.preventDefault();
    }
  };
...
/** Everything the board's own keydown wiring needs from `bootBoard`. */
export interface BoardShortcutWiring {
  document: Document;
  /** The board's one projection, handed straight to the handler. LIVE. */
  state: BoardShortcutDeps['state'] & { detailThreadId: string | null };
  /** `getElementById`, already narrowed — `bootBoard`'s own `el`. */
  el(id: string): HTMLElement;
  renderDetail(): void;
  /** The REST archive verb, already bound — `createBoardActions`' own. */
 ...
 */
export function wireBoardShortcuts(deps: BoardShortcutWiring): void {
  const { document, state, el, renderDetail, archiveTask, isArchived } = deps;
  document.addEventListener(
    'keydown',
    boardShortcutKeydown({
      state,
      helpEl: () => el('board-help'),
      openDetail: (taskId) => {
