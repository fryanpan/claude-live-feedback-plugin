 ...
 *
 * The bottom of the board's render layer: `board-discussion-render.ts` and
...
function assigneePicker(
  ...
  sel.className = `${className} board-owner-${kind}`;
  if (owner === '') {
 ...
 */
export function renderTaskLinks(task: BoardTask, workspaceId?: string): HTMLElement | null {
  const refs = Array.isArray(task.links) ? task.links : [];
  if (refs.length === 0) return null;
  const wrap = document.createElement('div');
  wrap.className = 'board-detail-links';
  ...
  for (const raw of refs) {
    ...
    if (kind === 'doc' && typeof ref.docId === 'string' && ref.docId !== '') {
      const a = document.createElement('a');
      a.className = `board-link-chip ${DOC_TITLE_LINK_CLASS}`;
    ...
    chip.className = 'board-link-chip board-link-internal';
...
export interface DetailHandlers {
   ...
   */
  onAnswer: (task: BoardTask, text: string, optionId?: string) => Promise<boolean> | undefined;
  /**
   ...
   */
  onAskOnPanelItem?: (task: BoardTask, item: PanelReviewItem, question: string) => Promise<boolean>;
   ...
   */
  onReleaseHeld?: (task: BoardTask, item: BoardReviewItem) => Promise<boolean> | undefined;
  /**
   ...
   */
  onUndoThreadAnswer?: (task: BoardTask, item: PanelReviewItem) => Promise<boolean> | undefined;
  /**
   ...
   *  `board-model`'s `goalLabel`, which resolves Backlog too. The panel
  ...
  onGoalSet?: (task: BoardTask, goalId: string) => void;
  /** Set the due date, or clear it with `null`. */
  onDueSet?: (task: BoardTask, dueAt: number | null) => void;
  /**
  ...
  onComment?: (task: BoardTask, text: string, threadId?: string) => Promise<boolean>;
  /**
  ...
  onActivityReply?: (task: BoardTask, threadId: string, text: string) => Promise<Thread | null>;
   ...
   */
  onCopyLink?: (task: BoardTask) => void;
  /**
   ...
   */
  onAsk?: (task: BoardTask, kind: TaskAskKind) => Promise<boolean>;
  /**
   ...
   */
  onBodySlot?: (task: BoardTask | null, slot: HTMLElement | null) => void;
}
...
export function activityRow(ev: ActivityEvent, title: string): HTMLLIElement {
  ...
  li.className = 'board-detail-activity-row';
...
export function renderTransitionRow(t: BoardTransition): HTMLLIElement {
  const li = document.createElement('li');
...
export function bodySlot(row: BodyRow): HTMLElement {
  ...
  if (row.body?.trim()) {
    desc.className = 'board-detail-body';
    desc.innerHTML = renderCommentMarkdown(row.body);
  } else {
    desc.className = 'board-detail-body-empty';
 ...
 */
export const BODY_LIVE_CLASS = 'board-detail-body-live';
/**
...
export function renderRelatedLinks(
  ...
  const wrap = document.createElement('div');
  wrap.className = 'board-related-links';
  const heading = document.createElement('p');
  heading.className = 'board-related-links-k';
  heading.textContent = 'Related Links';
  const list = document.createElement('ul');
  list.className = 'board-related-links-list';
  ...
  const remove = (li: HTMLElement, entry: RelatedEntry, reads: string): void => {
    ...
    x.type = 'button';
    x.className = 'board-related-link-x';
  ...
  for (const blocker of blockers) {
    const li = document.createElement('li');
    li.className = 'board-related-blocker';
    const mark = document.createElement('span');
    mark.className = 'board-status-mark board-status-mark-blocked board-related-mark';
    mark.setAttribute('aria-hidden', 'true');
    const a = document.createElement('a');
    a.className = 'board-related-link';
  ...
  for (const link of links) {
    const li = document.createElement('li');
    const a = document.createElement('a');
    a.className = `board-related-link ${DOC_TITLE_LINK_CLASS}`;
    ...
    if (link.held) {
      ...
      held.className = 'board-related-link-held';
  ...
  for (const url of urls) {
    const li = document.createElement('li');
    const a = document.createElement('a');
    a.className = 'board-related-link board-related-link-plain';
    a.href = url;
    a.rel = 'noreferrer';
    a.target = '_blank';
    a.textContent = url;
...
export function detailFields(
  ...
  dl.className = 'board-detail-fields';
  ...
  const cell = (key: string, value: Node | string, opts?: { full?: boolean }): void => {
    const wrap = document.createElement('div');
    wrap.className = opts?.full
      ? 'board-detail-field board-detail-field--full'
      : 'board-detail-field';
  ...
  mark.className = `board-status-mark board-status-mark-${blocked ? 'blocked' : task.status}`;
  mark.setAttribute('aria-hidden', 'true');
  const status = document.createElement('select');
  // Deliberately NOT `board-status-select` / `board-chip-<status>`. Those two are
  ...
  // so borrowing them would fight `.board-detail-select` for every property and
  // leave a dropdown with no caret.
  status.className = 'board-detail-select board-detail-status';
  ...
  cell(
    'Assignee',
    assigneePicker('board-detail-select board-assignee-btn', task, handlers.knownAgentIds, (to) =>
...
export function effortFields(
  ...
  detail.className = 'board-detail-field board-detail-effort-detail';
  detail.hidden = true;
  const detailBody = document.createElement('dd');
  detailBody.className = 'board-detail-field-v board-detail-effort-why';
  ...
  const value = (text: string): HTMLElement => {
    ...
    btn.type = 'button';
    btn.className = 'board-detail-effort-value';
    ...
    btn.addEventListener('click', () => {
      ...
      detail.hidden = !open;
      for (const other of [
        ...(btn.closest('dl')?.querySelectorAll('.board-detail-effort-value') ?? []),
      ]) {
...
export interface PanelReviewItem {
   ...
   *  (`board-app`), because a declaration with nowhere to record an answer is
