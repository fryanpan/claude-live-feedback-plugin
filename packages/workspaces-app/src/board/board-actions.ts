 ...
 *
 * `main()` in `board-app.ts` used to hold these as declarations inside a
 ...
 * identical from the outside. `createBoardActions` takes those captures as one
 * explicit object — `BoardActionDeps` — and hands back the same verbs. Nothing
 * about a write changed; what changed is that the file says what a write can
 * touch.
 *
 * `BoardState` lives here rather than in the entry because these verbs are its
 * writers: `board-review-controller.ts` and `board-live-wiring.ts` read the type
...
export function showToast(
  ...
  if (action) {
    ...
    btn.type = 'button';
    btn.className = 'board-toast-action';
...
export function createBoardActions(deps: BoardActionDeps) {
   ...
   */
  async function addRelatedLink(task: BoardTask, url: string): Promise<void> {
...
/**
 * Every verb `createBoardActions` hands back, as one type.
 *
