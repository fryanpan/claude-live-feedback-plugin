 ...
 * several tabs into one row (`foldTabs` in board-presence-model). An unchanged
...
function PresenceButton(props: {
  ...
  const base = compact ? 'board-presence-circle' : 'board-presence-chip';
  return (
    <button
      type="button"
      class={`${base} board-presence-${chip.kind}${chip.state ? ` board-presence-${chip.state}` : ''}${
        followed ? ' board-following' : ''
      }`}
    ...
    >
      {compact ? (
        <span class="board-presence-initials">{initialsOf(chip.label)}</span>
      ) : (
        <Fragment>
          <span class="board-presence-name">{chip.label}</span>
          <span class="board-presence-where">{chip.where}</span>
        </Fragment>
      )}
    </button>
  );
...
function OverflowCircle(props: { hidden: PresenceChip[]; handlers: PresenceHandlers }) {
  ...
  return (
    <button
      type="button"
      class="board-presence-circle board-presence-more"
      title={names}
    ...
    <Fragment>
      {notes.map(({ id, notice }) => (
        <div
          ...
          // past the alarm.
          class={notice.kind === 'coverage' ? 'board-drift board-drift-quiet' : 'board-drift'}
        >
          <span class="board-drift-head">{notice.headline}</span>
          <span class="board-drift-who">{notice.detail}</span>
          <span class="board-drift-fix">{notice.fix}</span>
        </div>
      ))}
    </Fragment>
  );
 ...
 *  items of the host, which is what the `.board-presence` layout is written
 *  against. */
