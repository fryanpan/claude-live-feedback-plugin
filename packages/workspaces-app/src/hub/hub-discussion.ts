 ...
 * editor through the same two functions. Scattered through `bootHub` that was
...
/** Everything the discussion needs from `bootHub`, and nothing else. */
export interface HubDiscussionDeps {
  /** The board's one projection: the load claims `discussionTaskId` on it and
   *  writes `discussion` back. LIVE, the same contract `wireHubLive` uses. */
  state: HubState;
  /** Who is posting. */
...
/** What `bootHub` keeps: the two verbs, plus the goal's room derivation. */
...
export function createHubDiscussion(deps: HubDiscussionDeps): HubDiscussion {
