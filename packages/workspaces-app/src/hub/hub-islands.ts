 ...
 * `buildShell` — the last vanilla write of `#hub-root` — and each is a mount
 ...
 * the region modules.
 *
 * `#hub-board` is deliberately NOT here: it is mounted at the very bottom of
 ...
 *
 * `HubIslandDeps` is the whole list of what a handler may reach. Every verb on
...
import type { ReviewItem } from './hub-review-model.ts';
...
/** Everything the islands' handlers need from `bootHub`, and nothing else. */
export interface HubIslandDeps {
 ...
 * region.
 */
export function mountHubIslands(deps: HubIslandDeps): void {
  const {
    state,
    user,
    el,
