 ...
 * that hide with the rows (`hub-quick`, `hub-decisions`, `hub-archived`) are
 ...
 *
 * `HubBoardDeps` is the whole list of what this pane may reach. The REST verbs
...
import type { HubActions, HubState } from './hub-actions.ts';
import {
  type BoardSection,
  type HubTask,
  type ReorderTarget,
...
/** Everything the board pane needs from `bootHub`, and nothing else. */
export interface HubBoardDeps {
  ...
  setNav(nav: HubNav): void;
}

/** What `bootHub` keeps: the four renders and the openers around them. */
export interface HubBoardRegion {
  /** The one opener behind every task tap. */
  openTaskDetail(task: HubTask, tab?: DetailTab): void;
...
export function createHubBoardRegion(deps: HubBoardDeps): HubBoardRegion {
  const {
    state,
    user,
    el,
