 ...
 * together they are a small state machine; read scattered through `bootHub`
 ...
 * and `walkSources` stay in `bootHub`, because they are wired from the
 ...
 *
 * The store holds no board: `HubWalkthroughDeps` is the whole list of what a
 * walk verb may reach. The three review-write callbacks arrive as thunks
 * because `createHubReviewController` takes `render` as one of ITS inputs —
...
/** Everything the walkthrough needs from `bootHub`, and nothing else. */
export interface HubWalkthroughDeps {
  /** The board's one projection. The walk reads and writes four fields on
   *  it — `walkIndex`, `walkKey`, `walkProgress`, `homeSettled` — and the
   *  object is LIVE, the same contract `wireHubLive` is written against. */
  state: HubState;
  ...
  /** The three review writes, from `createHubReviewController`. Thunks, not
  ...
  /** This board's queue just drained. `bootHub` decides whether the sitting
   *  continues on another board (`?then=`) or ends here. */
  onQueueDrained(): void;
}

/** What `bootHub` keeps: the two verbs the rest of the page calls. */
export interface HubWalkthrough {
...
export function createHubWalkthrough(deps: HubWalkthroughDeps): HubWalkthrough {
  const {
    state,
    currentQueue,
    el,
