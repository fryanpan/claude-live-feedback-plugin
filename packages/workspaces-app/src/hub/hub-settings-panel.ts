 ...
 * the middle of `bootHub`'s wiring section before this file existed.
 *
 ...
 *
 * Nothing here can reach the board: `HubSettingsPanelDeps` is the whole list,
 * and the open flag arrives as a two-thunk accessor rather than as the
 * `HubState` it actually lives on.
 */
...
/** Everything the settings panel needs from `bootHub`, and nothing else. */
export interface HubSettingsPanelDeps {
  document: Document;
  /** `getElementById`, already narrowed to a present element — `bootHub`'s
  ...
  /** The panel's open flag, which lives on `HubState`. Two thunks rather
   *  than the state object: this file has no other business with it. */
  isOpen(): boolean;
  setOpen(open: boolean): void;
  /** Paint the open/closed state. `bootHub` owns the render layer. */
 ...
 *
 * Called once, from `bootHub`'s wiring section, in the same position the
 * inlined block held — the click listeners it registers on `document` are
 * order-sensitive against the ones around them.
 */
export function wireHubSettingsPanel(deps: HubSettingsPanelDeps): void {
