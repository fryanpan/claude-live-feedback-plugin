 ...
 * `hub-actions.ts`: `createHubReviewController` takes what they captured as one
...
import { send, showToast } from './hub-actions.ts';
import { type HubReviewItem, type HubTask } from './hub-board-model.ts';
 ...
 */
export interface HubReviewControllerDeps {
  ...
  loadDiscussion: (task: HubTask, quiet?: boolean) => Promise<void>;
  /** Open a task's panel on one thread — the 409 toast's way out. */
  openTaskThread: (taskId: string, threadId: string) => boolean;
}

export function createHubReviewController(deps: HubReviewControllerDeps) {
  const {
    author,
    state,
    currentQueue,
