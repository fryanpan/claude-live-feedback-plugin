 ...
 *
 * `HubLoadDeps` is the whole list of what these reads may reach.
 */
import type { HubState } from './hub-actions.ts';
import { fetchJson } from './hub-actions.ts';
import {
  type ActivityEvent,
  type ClientRelease,
  type LeadSeatView,
...
import { type ReviewThreadItem, applyRefresh, refreshReviewItems } from './hub-review-model.ts';

/** Everything the loads need from `bootHub`, and nothing else. */
...
/** What `bootHub` keeps: the three loads, plus the one repaint closure the
 *  live wiring schedules on its own. */
export interface HubLoads {
  loadReviewItems(): Promise<void>;
  loadAgents(): Promise<void>;
  loadEvents(): Promise<void>;
  /** The three regions the tasks projection and the review-items list feed.
   *  Handed to `wireHubLive`, which schedules it on every ydoc move. */
  repaintQueueRegions: () => void;
}

export function createHubLoads(deps: HubLoadDeps): HubLoads {
  const {
    state,
    workspaceId,
    schedule,
