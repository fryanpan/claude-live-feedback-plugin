 ...
 * `?task=`, `?goal=`, `?thread=` and `?item=` are written into `HubState`
 * before the first paint (see `bootLoc` in `bootHub`), so what is left is the
 ...
 * claim behind — which is why `bootHub` keeps the three pending `let`s next
...
import { type HubState, showToast } from './hub-actions.ts';
import { goalSection } from './hub-board-model.ts';
import {
  type ReviewItem,
  type ReviewQueue,
  type WalkSources,
...
/** The boot URL's unresolved claims, owned by `bootHub` because the address
...
/** Everything the waiting needs from `bootHub`, and nothing else. */
export interface HubDeepLinkDeps {
  ...
  /** Which halves of the queue have landed. LIVE — `bootHub` writes it from
  ...
  syncBoardUrl(): void;
  renderDetail(): void;
  /** The three pending claims `bootHub` holds beside the address bar. */
  boot: BootClaims;
}

/** What `bootHub` keeps. */
export interface HubDeepLinks {
   ...
   *  in the `?then=` chain. `bootHub` gives this to the walkthrough. */
  chainWalkDrain(): void;
}

export function createHubDeepLinks(deps: HubDeepLinkDeps): HubDeepLinks {
  const {
    state,
    bootLoc,
    location,
