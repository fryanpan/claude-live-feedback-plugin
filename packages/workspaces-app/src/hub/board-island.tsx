...
function TaskBadges(props: {
    ...
    badges.push(
      rule ? (
        <button
          key="recur"
          type="button"
          class="hub-recur hub-recur-link"
  ...
  if (chips) {
    ...
    if (chips.next !== undefined) {
      badges.push(
        <span key="next" class={chips.soon ? 'hub-next hub-next-soon' : 'hub-next'}>
          {chips.next}
        </span>,
      );
    }
  ...
  return (
    <div
      class={[
        ...
        section.isScheduled === true ? 'hub-band-scheduled' : '',
      ...
      {sections.map((section) => (
        <section
          ...
          class={`hub-section${section.isChores ? ' hub-chores' : ''}${
            section.isScheduled === true ? ' hub-scheduled' : ''
          }`}
