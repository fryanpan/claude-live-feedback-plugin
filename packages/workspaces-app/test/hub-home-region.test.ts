...
import { homeActivityData } from '../src/hub/home-activity-island.tsx';
import { homeReviewData } from '../src/hub/home-review-island.tsx';
import { createHubHomeRegion } from '../src/hub/hub-home-region.ts';
 ...
 * crossed sixty lines while this lived in `bootHub`: the poll that waits for
 ...
 */
function home(over: Partial<Parameters<typeof createHubHomeRegion>[0]> = {}) {
  const el = mountShell();
  const state = hubState({ pane: 'home', nav: 'home', tasks: new Map([['t-1', task('t-1')]]) });
  const scheduled: Array<() => void> = [];
  const api = createHubHomeRegion({
    state,
    workspaceId: 'w-1',
...
describe('createHubHomeRegion', () => {
  it('marks the destination the reader is on, and only that one', () => {
    const h = home();
    h.renderHomeRegion();
    const active = [...document.querySelectorAll('.hub-nav-item')].filter((b) =>
      b.classList.contains('hub-nav-item-active'),
    );
  ...
  it('draws nothing into the pane while the reader is on the board', () => {
    ...
    h.renderHomeRegion();
    expect(h.el('hub-home').classList.contains('hidden')).toBe(true);
