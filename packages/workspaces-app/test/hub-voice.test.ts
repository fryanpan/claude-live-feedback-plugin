...
import { wireHubVoice } from '../src/hub/hub-voice.ts';
...
function voice(over: Partial<Parameters<typeof wireHubVoice>[0]> = {}) {
  const el = mountShell();
...
async function utter(v: ReturnType<typeof voice>, text: string): Promise<void> {
  v.el('hub-mic').dispatchEvent(new Event('pointerdown'));
  v.rec.say(text);
  v.el('hub-mic').dispatchEvent(new Event('pointerup'));
