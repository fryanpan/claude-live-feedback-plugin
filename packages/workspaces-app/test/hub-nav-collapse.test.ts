...
import { wireNavCollapse } from '../src/hub/hub-shell.ts';
import { mountShell } from './support/hub-region-harness.ts';
...
describe('wireNavCollapse', () => {
  ...
  it('starts expanded when nothing was stored', () => {
    ...
    expect(el('hub-nav-collapse').title).toBe('Collapse');
  });
  ...
  it('swaps the button’s glyph, so the control says which way it goes', () => {
    wireNavCollapse(document, storage());
    const glyph = () => el('hub-nav-collapse').querySelector('.hub-nav-icon')?.innerHTML ?? '';
