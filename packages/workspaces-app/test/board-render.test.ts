...
import { BODY_LIVE_CLASS, type TaskThread } from '../src/board/board-detail-render.ts';
import { discussionIsBusy, flattenComments } from '../src/board/board-discussion-render.ts';
import { type BoardGoal, type BoardTask, CHORES_ID, goalLabel } from '../src/board/board-model.ts';
import { type UptimeReport } from '../src/board/board-presence-model.ts';
import {
  renderActivity,
  renderHomeBrief,
  renderLeadStrip,
  renderReviewBanner,
} from '../src/board/board-render.ts';
import {
...
import {
  ...
  panelReviewQueue,
} from '../src/board/board-review-render.ts';
...
describe('renderReviewBanner', () => {
  it('renders one line and a way to Home while items are open, nothing at all when none are', () => {
    ...
    expect(root.querySelector('.board-review-banner-text')?.textContent).toBe(
      'Something is waiting for your review',
    );
    (root.querySelector('.board-review-banner-go') as HTMLElement).click();
  ...
  it('still renders one countless line when several kinds are waiting', () => {
    ...
    const text = root.querySelector('.board-review-banner-text')?.textContent;
...
describe('renderHomeBrief', () => {
  ...
  it('renders the brief as markdown under "What\'s New?", with the window in the head row', () => {
    ...
    expect(root.querySelector('.board-home-heading')?.textContent).toBe("What's New?");
    expect(root.querySelector('.board-home-brief-body strong')?.textContent).toBe('Finished:');
    ...
    const since = root.querySelector('.board-home-review-head .board-home-since');
  ...
  it('generating appends Updating… to the window line', () => {
    ...
    const since = root.querySelector('.board-home-since')?.textContent ?? '';
  ...
  it('Mark read is the dark button on the right; the edit link sits left and opens the editor', () => {
    ...
    const mark = root.querySelector('.board-home-mark-read') as HTMLElement;
    ...
    expect(mark.className).toContain('board-btn-ink');
    mark.click();
    expect(onMarkCaughtUp).toHaveBeenCalledTimes(1);
    const actions = root.querySelector('.board-home-brief-actions') as HTMLElement;
    // DOM order: link first (left), Mark read last (right).
    expect(actions.firstElementChild?.classList.contains('board-home-edit-recipe')).toBe(true);
    expect(actions.lastElementChild).toBe(mark);
    expect(root.querySelector('.board-home-edit-recipe')?.textContent).toBe(
      'Edit how this gets generated',
    );
    (root.querySelector('.board-home-edit-recipe') as HTMLElement).click();
    expect(onEditRecipe).toHaveBeenCalledWith(true);
    // Closed by default: the panel only exists when the app says it is open.
    expect(root.querySelector('.board-home-recipe')).toBeNull();
  });
  ...
  it('the open recipe editor carries the exact approved copy and exactly two buttons', () => {
    ...
    expect(root.querySelector('.board-home-recipe-hint')?.textContent).toBe(
      'Edit these instructions and they will be used on this summary and future summaries.',
    );
    const ta = root.querySelector('.board-home-recipe-text') as HTMLTextAreaElement;
    expect(ta.value).toBe('Under 200 words.');
    const buttons = root.querySelectorAll('.board-home-recipe button');
    expect([...buttons].map((b) => b.textContent)).toEqual(['Save & Update Summary', 'Cancel']);
    ta.value = 'Be terse.';
    (root.querySelector('.board-home-recipe-save') as HTMLElement).click();
    expect(onSaveInstructions).toHaveBeenCalledWith('Be terse.');
    (root.querySelector('.board-home-recipe-cancel') as HTMLElement).click();
  ...
  it('a blank instructions box saves nothing — blanking the recipe is not expressible', () => {
    ...
    (root.querySelector('.board-home-recipe-text') as HTMLTextAreaElement).value = '   ';
    (root.querySelector('.board-home-recipe-save') as HTMLElement).click();
  ...
  it('no payload yet renders a loading line, not an empty card', () => {
    ...
    expect(root.querySelector('.board-home-quiet')?.textContent).toBe('Loading…');
  });
});
...
describe('renderLeadStrip', () => {
  it('names the lead and lists every known agent as a reassignment target', () => {
    ...
    expect(root.classList.contains('board-lead-empty')).toBe(false);
  ...
  it('an empty seat reads as a state to fix, and still offers the attached agents', () => {
    ...
    expect(root.classList.contains('board-lead-empty')).toBe(true);
    const select = root.querySelector('select') as HTMLSelectElement;
  ...
  it('a seat whose holder stopped answering is drawn as loudly as an empty one', () => {
    ...
    expect(root.classList.contains('board-lead-stale')).toBe(true);
  ...
  it('POSITIVE CONTROL: a live lead is drawn exactly as it was before the seat existed', () => {
    ...
    expect(root.classList.contains('board-lead-stale')).toBe(false);
    expect(root.textContent).toContain('Lead agent');
...
describe('renderActivity', () => {
  ...
  it('has exactly two filters and swaps rows between them', () => {
    ...
    const tabs = Array.from(root.querySelectorAll('.board-activity-filters .board-tab'));
  ...
  it('renders the uptime banner — ok styling when the 99% target is met', () => {
    ...
    const banner = root.querySelector('.board-uptime');
    expect(banner).not.toBeNull();
    expect(banner?.classList.contains('board-uptime-ok')).toBe(true);
  ...
  it('a missed target gets the miss styling and shows the downtime', () => {
    ...
    const banner = root.querySelector('.board-uptime');
    expect(banner?.classList.contains('board-uptime-miss')).toBe(true);
  ...
  it('no report, no banner (the two tests above are the presence control)', () => {
    ...
    expect(root.querySelector('.board-uptime')).toBeNull();
  });
});
...
describe('renderTaskDetail', () => {
  ...
  it('shows the blocked note, with the count and the names, when the open task holds work up', () => {
    ...
    const note = root.querySelector<HTMLElement>('.board-blocked-note');
    ...
    expect(note?.querySelector('.board-decide-k')?.textContent).toBe('Blocking');
    ...
    expect(note?.previousElementSibling?.className).toContain('board-detail-fields');
  });
  ...
  it('renders no blocked note on an agent task or a task nothing waits on', () => {
    ...
    for (const open of [agentGate, idle]) {
      ...
      expect(root.querySelector('.board-blocked-note')).toBeNull();
    }
  });
  ...
  it('takes focus on open and declares itself page-like for the Space hold', () => {
    ...
    const panel = root.querySelector<HTMLElement>('.board-detail-panel');
  ...
  it('does not re-take focus on a repaint of the same task', async () => {
    ...
    const ta = root.querySelector<HTMLTextAreaElement>('.board-detail-panel textarea');
    ...
    renderTaskDetail(root, t, withComposer(), discussion);
    await frame();
    const panel = root.querySelector('.board-detail-panel');
    const rebuilt = root.querySelector<HTMLTextAreaElement>('.board-detail-panel textarea');
  ...
  const metaValue = (key: string): string | null => {
    const dts = [...root.querySelectorAll('.board-detail-fields dt')];
    const dds = [...root.querySelectorAll('.board-detail-fields dd')];
  ...
  describe('the history rows say who moved the task and what they said', () => {
    const moved = (t: Partial<BoardTask['transitions'][number]>) =>
      task({
        status: 'done',
        transitions: [
          {
    ...
    const rows = () => [...root.querySelectorAll('.board-detail-transitions > li')];
  ...
  it('renders a url ref as a real anchor', () => {
    ...
    const chip = root.querySelector('.board-detail-links a') as HTMLAnchorElement;
  ...
  it('never emits a non-http(s) href, even for a ref stored before the check existed', () => {
    ...
    expect(root.querySelectorAll('.board-detail-links a').length).toBe(1);
    ...
    for (const url of ['javascript:alert(1)', 'data:text/html,<script>x</script>']) {
      ...
      expect(root.querySelectorAll('.board-detail-links a').length).toBe(0);
    }
  });
  ...
  it('shows the doc TITLE for a doc-kind link, basename-fallback included — never a raw id', () => {
    ...
    const anchors = [...root.querySelectorAll<HTMLAnchorElement>('.board-detail-links a')];
  ...
  it('shows the answer form for an unanswered decision and records verbatim text', () => {
    ...
    const ta = root.querySelector('.board-answer-form textarea') as HTMLTextAreaElement;
    ta.value = 'Go with option B, ship Thursday.';
    (root.querySelector('.board-answer-form') as HTMLFormElement).dispatchEvent(
  ...
  it("says Answered by you when the task's own answer is the reader's — same voice as a thread record", () => {
    ...
    expect(root.querySelector('.board-detail-answer')?.textContent).toBe(
  ...
  it("names who filed the ticket on the task's own decision card, as a thread card names its asker", () => {
    ...
    expect(root.querySelector('.board-decide-meta')?.textContent).toBe(
      'Asked by UX Bot 1 hour ago',
    );
  });
  ...
  it('renders the recorded answer instead of the form once answered', () => {
    ...
    expect(root.querySelector('.board-answer-form')).toBeNull();
    expect(root.querySelector('.board-detail-answer')?.textContent).toContain('Option B');
  });
  ...
  it('renders the description on the task itself, as markdown', () => {
    ...
    const desc = root.querySelector('.board-detail-body');
  ...
  it('labels a preserved capture so it reads as provenance, not as a stray quote', () => {
    ...
    const fig = root.querySelector('.board-detail-quote-block');
    expect(fig).toBeTruthy();
    expect(fig?.querySelector('.board-detail-quote-label')?.textContent).toBe('Original words');
    // The words themselves survive the wrapper — the label must not be the
    // only thing that made it into the DOM.
    expect(fig?.querySelector('.board-detail-quote')?.textContent).toContain(
    ...
    expect(
      root.querySelector('.board-detail-quote-label')?.closest('.board-detail-quote-block'),
    ).toBe(fig);
  });
  ...
  it('keeps the preserved capture reachable but below the description, closed by default', () => {
    ...
    const quote = root.querySelector('.board-detail-quote-block') as HTMLDetailsElement;
    const desc = root.querySelector('.board-detail-body');
  ...
  it('keeps the capture open across a repaint once the reader opens it', () => {
    ...
    renderTaskDetail(root, t, detailHandlers());
    expect((root.querySelector('.board-detail-quote-block') as HTMLDetailsElement).open).toBe(true);
    ...
    expect((root.querySelector('.board-detail-quote-block') as HTMLDetailsElement).open).toBe(
      false,
    );
  });
  ...
  it('shows no quote block at all on a task that never had one', () => {
    ...
    expect(root.querySelector('.board-detail-quote-label')).toBeTruthy();
    ...
    expect(root.querySelector('.board-detail-quote-block')).toBeNull();
    expect(root.querySelector('.board-detail-quote-label')).toBeNull();
    // …on a panel that did render: the description is right there.
    expect(root.querySelector('.board-detail-body')?.textContent).toContain('no captured words');
  });
  ...
  it('escapes markup in a description rather than executing it', () => {
    ...
    expect(root.querySelector('.board-detail-body img')).toBeNull();
    expect(root.querySelector('.board-detail-body')?.textContent).toContain('<img');
  });
  ...
  it('says a task has no description rather than showing nothing', () => {
    ...
    expect(root.querySelector('.board-detail-body-empty')).toBeNull();

    renderTaskDetail(root, task(), detailHandlers());
    expect(root.querySelector('.board-detail-body-empty')).toBeTruthy();
    // The link out stays either way — the doc is where you edit and comment.
    expect(root.querySelector('.board-detail-body-link a')).toBeTruthy();
  });
  ...
  it('says so when the projected description is only the head of a longer one', () => {
    ...
    expect(root.querySelector('.board-detail-body-more')).toBeNull();
    ...
    expect(root.querySelector('.board-detail-body-more')).toBeTruthy();
  });
  ...
  it('the assignee row picks who takes it — the same choice the board row offers', () => {
    ...
    const pick = root.querySelector('.board-assignee-btn') as HTMLSelectElement;
...
describe('renderTaskDetail — discussion', () => {
  ...
  it('does not scroll past the review queue to the thread it already quotes', () => {
    ...
    expect(root.querySelector('.board-comment-focus')).toBeTruthy();
    expect(root.querySelector('.board-decide-card[data-review-thread-id="th-1"]')).toBeTruthy();
  ...
  it('states the ask at the top of the panel, above the description', () => {
    ...
    const ask = root.querySelector('.board-decide');
    expect(ask).toBeTruthy();
    expect(ask?.querySelector('.board-decide-headline')?.textContent).toContain(
  ...
  it('replies to the asking thread from the review card itself', async () => {
    ...
    const form = root.querySelector('.board-decide-form') as HTMLFormElement;
  ...
  it('renders a ticket-borne review item and answers it with its review item id', async () => {
    ...
    const card = root.querySelector<HTMLElement>('.board-decide-card');
    expect(card?.dataset.reviewItemId).toBe('task-review:t-1:r-1');
    expect(card?.querySelector('.board-decide-headline')?.textContent).toBe(
      'Which cache do we keep?',
    );
    expect(card?.querySelector('.board-decide-body')?.textContent).toContain('Disk survives');
    const options = [...root.querySelectorAll<HTMLElement>('.board-decide-option')];
  ...
  it('says a revised ticket-borne item came back changed, and quotes what was asked', () => {
    ...
    const card = root.querySelector<HTMLElement>('.board-decide-card');
    expect(card?.querySelector('.board-decide-k-revised')?.textContent).toBe('Revised');
    expect(card?.querySelector('.board-decide-question')?.textContent).toBe(
    ...
    expect(root.querySelector('.board-decide-k-revised')).toBeNull();
    expect(root.querySelector('.board-decide-question')).toBeNull();
  });
  ...
  describe('"I have a question" on a ticket-borne card', () => {
    ...
    const link = () => root.querySelector<HTMLElement>('.board-decide-question-link');
    const box = () => root.querySelector<HTMLElement>('.board-decide-question-box');
    const answering = () => root.querySelector<HTMLElement>('.board-decide-answering');
    ...
    it('shows the link under the answer box when the surface can ask', () => {
      ...
      expect(l?.closest('.board-decide-card')).not.toBeNull();
      expect(box()).toBeNull();
    ...
    it('draws no link without a handler, nor on a thread-borne card — which says why', () => {
      ...
      expect(root.querySelector('.board-decide-card')).not.toBeNull();
      expect(link()).toBeNull();
      expect(root.querySelector('.board-decide-question-note')?.textContent).toContain(
        'Reply above',
      );
    });
    ...
    it('the ticket’s own decision card offers the link, and Send asks through the panel handler', async () => {
      ...
      const l = link();
      expect(l?.textContent).toBe('I have a question');
      expect(root.querySelector('.board-decide-question-note')).toBeNull();
      l?.click();
      const form = root.querySelector('.board-decide-question-form') as HTMLFormElement;
      expect(form.closest('.board-decide-question-box')?.textContent).toContain('Ask Poller Bot');
    ...
    it('tapping it turns the card into a question box — textarea, Send, Cancel — hiding the answer furniture', () => {
      ...
      expect(b?.querySelector('.board-decide-question-form textarea')).not.toBeNull();
      expect(
        b?.querySelector('.board-decide-question-form button[type="submit"]')?.textContent,
      ).toBe('Send');
      expect(b?.querySelector('.board-decide-question-cancel')?.textContent).toBe('Cancel');
      expect(b?.querySelector('.board-decide-form-hint')?.textContent).toContain('Ask Helper');
      // Hidden, not unmounted: the answer box keeps its draft.
      expect(answering()?.classList.contains('hidden')).toBe(true);
      expect(root.querySelector('.board-decide-form textarea')).not.toBeNull();
      (b?.querySelector('.board-decide-question-cancel') as HTMLElement).click();
      expect(box()).toBeNull();
      expect(answering()?.classList.contains('hidden')).toBe(false);
    });
    ...
    it('opening the box puts the caret in it — the link that opened it has left the tab order', async () => {
      ...
      expect(document.activeElement?.closest('.board-decide-card')).toBeNull();
      link()?.click();
      ...
      // textarea until then.
      const form = box()?.querySelector<HTMLFormElement>('.board-decide-question-form');
      expect(form).not.toBeNull();
      await new Promise((r) => setTimeout(r, 30));
      expect(document.activeElement?.closest('.board-detail-panel')).not.toBeNull();
    ...
    it('POSITIVE CONTROL: the answer path is untouched — a tapped option still answers', () => {
      ...
      [...root.querySelectorAll<HTMLElement>('.board-decide-option')][1]?.click();
      expect(onAnswerThread).toHaveBeenCalledWith(
        expect.objectContaining({ id: 't-1' }),
        expect.objectContaining({ source: 'task-review', reviewItemId: 'r-1' }),
        'Keep memory',
  ...
  /**
   * The card sits directly under the panel's own `.board-detail-title`, so a
  ...
  it('drops a headline that only repeats the ticket title the panel already shows', () => {
    ...
    expect(root.querySelector('.board-detail-title')?.textContent).toBe(
      'Rename the catch-all band',
    );
    expect(root.querySelector('.board-decide-headline')).toBeNull();
    // Nothing was swallowed with it: the card still carries the body, which
    // is the condition the drop is gated on.
    expect(root.querySelector('.board-decide-body')?.textContent).toContain('Two bands answer');
  });
  ...
  it('keeps a headline that says more than the ticket title', () => {
    ...
    expect(root.querySelector('.board-decide-headline')?.textContent).toBe(
  ...
  it('heads an item nobody was named on with the plain review heading', () => {
    ...
    const kicker = root.querySelector('.board-decide-kicker')?.textContent ?? '';
    ...
    expect(root.querySelector('.board-decide-headline')?.textContent).toContain('PR #154');
  });
  ...
  it('calls a direct question a question', () => {
    ...
    expect(root.querySelector('.board-decide-kicker')?.textContent).toContain(
  ...
  it('renders the declared review payload as one markdown body, links and all', () => {
    ...
    const card = root.querySelector('.board-decide-card');
    expect(card).toBeTruthy();
    expect(card?.querySelector('.board-decide-headline')?.textContent).toContain('rollup query');
    // ONE body (approved design): the payload's `detail`, markdown-rendered,
    // no labelled sub-sections and none of the old paragraphs.
    expect(card?.querySelector('.board-decide-why')).toBeNull();
    expect(card?.querySelector('.board-decide-lookfor')).toBeNull();
    expect(card?.querySelector('.board-decide-detail')).toBeNull();
    const body = card?.querySelector('.board-decide-body') as HTMLElement;
    ...
    expect(card?.querySelector('.board-decide-k')?.textContent).toBe('Question');
    expect(card?.querySelector('.board-decide-k')?.className).toContain('board-decide-k-review');
  });
  ...
  it('offers a declared item’s options as one-tap answers on its own thread', () => {
    ...
    const opts = root.querySelectorAll('.board-decide-card .board-decide-option');
    expect(opts).toHaveLength(2);
    expect(opts[0]?.querySelector('.board-decide-option-label')?.textContent).toBe(
      'Keep threading',
    );
    expect(opts[0]?.querySelector('.board-decide-option-detail')?.textContent).toBe(
      'Costs a migration.',
    );
    // The second has no detail, so no detail element — not an empty one.
    expect(opts[1]?.querySelector('.board-decide-option-detail')).toBeNull();
    ...
    const form = root.querySelector('.board-decide-form') as HTMLFormElement;
    expect(form).toBeTruthy();
    expect(form.querySelector('.board-decide-form-hint')?.textContent).toContain('your own words');
  });
  ...
  it('leaves an undeclared ask exactly as it was', () => {
    ...
    const card = root.querySelector('.board-decide-card');
    expect(card?.querySelector('.board-decide-headline')?.textContent).toContain(
      'should we drop threading',
    );
    // Nothing declared means nothing composed: no body at all, not an empty one.
    expect(card?.querySelector('.board-decide-body')).toBeNull();
    expect(card?.querySelectorAll('.board-decide-option')).toHaveLength(0);
    const form = root.querySelector('.board-decide-form') as HTMLFormElement;
    expect(form.querySelector('.board-decide-form-hint')?.textContent).toBe(
  ...
  it('shows each comment author and time as text, and leaves waiting to the queue', () => {
    ...
    const waiting = root.querySelector('.board-comment[data-thread-id="th-w"]');
    expect(waiting?.querySelector('.board-comment-author')?.textContent).toBe('Live Feedback');
    // Text, not a tooltip.
    expect(waiting?.querySelector('.board-comment-when')?.textContent).toBe('2h ago');
    ...
    expect(root.querySelectorAll('.board-comment [class*="needs"]')).toHaveLength(0);
    const carded = [...root.querySelectorAll<HTMLElement>('.board-decide-card')].map(
  ...
  it('sets a declared comment apart, with its header above the words', () => {
    ...
    const comments = [...root.querySelectorAll('.board-comment')];
    // The declaration rides the comment that made it, so the status note
    // above it stays a status note.
    expect(comments.map((c) => c.className.includes('board-comment-review'))).toEqual([
      false,
      true,
    ]);
    const declared = comments[1] as HTMLElement;
    expect(declared.querySelector('.board-comment-review-k')?.textContent).toBe('Decision');
    expect(declared.querySelector('.board-comment-review-headline')?.textContent).toBe(
    ...
    expect(declared.querySelector('.board-comment-review-why')).toBeNull();
    // Above the words, not instead of them — the text is what the agent said.
    expect(declared.querySelector('.board-comment-body')?.textContent).toContain(
  ...
  it('marks a withdrawn declaration instead of badging it as a question', () => {
    ...
    const declared = root.querySelector('.board-comment') as HTMLElement;
    expect(declared.querySelector('.board-comment-review-k')?.textContent).toBe('Withdrawn');
    expect(
      declared.querySelector('.board-comment-review-k')?.classList.contains('is-withdrawn'),
    ).toBe(true);
    // The words stay legible — struck, not removed.
    const headline = declared.querySelector('.board-comment-review-headline');
  ...
  it('marks the thread the queue aimed at, and only that one', () => {
    ...
    const marked = [...root.querySelectorAll('.board-comment-focus')];
    // Positive control: all three rendered, so "only one marked" means
    // something. Then: it is the RIGHT one.
    expect(root.querySelectorAll('.board-comment')).toHaveLength(3);
  ...
  it('marks nothing when the panel was opened any other way', () => {
    ...
    expect(root.querySelectorAll('.board-comment')).toHaveLength(2);
    expect(root.querySelectorAll('.board-comment-focus')).toHaveLength(0);
  });
  ...
  it('offers a composer on a task with no description and no comments', () => {
    ...
    const form = root.querySelector('.board-comment-form') as HTMLFormElement;
    expect(form).toBeTruthy();
    const ta = form.querySelector('textarea') as HTMLTextAreaElement;
  ...
  it('offers exactly one composer, whatever the thread count', () => {
    for (const threads of [
      ...
      renderTaskDetail(root, task(), detailHandlers(), { loading: false, threads });
      expect(root.querySelectorAll('.board-discussion textarea')).toHaveLength(1);
    }
  ...
  it('sends a comment to the newest conversation, with nothing on screen saying so', () => {
    ...
    expect(root.querySelector('.board-comment-reply')).toBeNull();
    expect(root.querySelector('.board-composer-target')).toBeNull();
    expect(root.querySelector('.board-composer-switch')).toBeNull();
    const form = root.querySelector('.board-comment-form') as HTMLFormElement;
    const ta = form.querySelector('textarea') as HTMLTextAreaElement;
  ...
  it('Cmd+Enter posts the comment', () => {
    const onComment = vi.fn();
    renderTaskDetail(root, task(), detailHandlers({ onComment }), { loading: false, threads: [] });
    const ta = root.querySelector('.board-comment-form textarea') as HTMLTextAreaElement;
  ...
  it('Cmd+Enter on an empty box refuses with the note, same as the button', () => {
    ...
    expect(onComment).not.toHaveBeenCalled();
    expect(root.querySelector('.board-comment-form .board-form-error')?.textContent).toBe(
  ...
  it('puts every thread’s comments in one stream, with nothing telling them apart', () => {
    ...
    const rows = [...root.querySelectorAll<HTMLElement>('.board-comment')];
    ...
    expect(root.querySelector('.board-comment-anchor')).toBeNull();
    // One box for all three, at the end of the stream.
    const boxes = [...root.querySelectorAll('.board-discussion textarea')];
    expect(boxes).toHaveLength(1);
    const form = root.querySelector('.board-comment-form') as HTMLElement;
  ...
  it('keeps a resolved thread’s words in the stream, with no status chrome', () => {
    ...
    const el = root.querySelector('.board-comment') as HTMLElement;
    expect(el).toBeTruthy();
    expect(el.textContent).toContain('Is the index really first?');
    expect(el.classList.contains('board-comment-resolved')).toBe(false);
    expect(el.querySelector('.board-comment-status')).toBeNull();
  });
  ...
  it('distinguishes "still loading" from "nothing to say yet"', () => {
    ...
    expect(root.querySelector('.board-discussion-loading')).toBeTruthy();
    expect(root.querySelector('.board-discussion-empty')).toBeNull();

    renderTaskDetail(root, task(), detailHandlers(), { loading: false, threads: [] });
    expect(root.querySelector('.board-discussion-loading')).toBeNull();
    expect(root.querySelector('.board-discussion-empty')).toBeTruthy();
  });
  ...
  it('renders comment text as inert markup', () => {
    ...
    const body = root.querySelector('.board-comment-body') as HTMLElement;
  ...
  // the board renders detail before the threads have been fetched.
  it('renders with no discussion supplied', () => {
    renderTaskDetail(root, task({ title: 'Wire the index' }), detailHandlers());
    expect(root.querySelector('.board-detail-title')?.textContent).toBe('Wire the index');
    expect(root.querySelector('.board-comment-form')).toBeNull();
  });
});
...
describe('discussionIsBusy', () => {
  ...
  it('is quiet when the composer is empty and unfocused', () => {
    open();
    expect(root.querySelector('.board-discussion textarea')).toBeTruthy();
    expect(discussionIsBusy(root)).toBe(false);
  });

  it('is busy while a draft is sitting in the composer', () => {
    open();
    const ta = root.querySelector('.board-discussion textarea') as HTMLTextAreaElement;
...
describe('a repaint of the detail panel keeps what was typed', () => {
  ...
  const paint = (t: BoardTask, extra: Record<string, unknown> = {}) =>
    renderTaskDetail(
      root,
      t,
      { ...handlers(), ...extra },
  ...
  const composer = () => root.querySelector('.board-discussion textarea') as HTMLTextAreaElement;
  ...
  it('the discussion composer survives a task transition — text, focus AND caret', async () => {
    ...
    expect((root.querySelector('.board-detail-status') as HTMLSelectElement).value).toBe(
      'in-progress',
    );
  ...
  it('the review card’s answer box survives too', async () => {
    ...
    const box = root.querySelector('.board-decide-form textarea') as HTMLTextAreaElement;
    expect(box).toBeTruthy();
    await typeInComposer(box, 'Keep threading.', 4);
    paint({ ...t, status: 'in-progress' }, { asks: [ask(t.id, 'th-1')] });
    await frame();
    const after = root.querySelector('.board-decide-form textarea') as HTMLTextAreaElement;
  ...
  it('a decision answer being recorded survives too', async () => {
    ...
    paint(t);
    const box = root.querySelector('.board-answer-form textarea') as HTMLTextAreaElement;
    expect(box).toBeTruthy();
    await typeInComposer(box, 'Option B, because', 8);
    paint({ ...t, updatedAt: NOW + 1 });
    await frame();
    const after = root.querySelector('.board-answer-form textarea') as HTMLTextAreaElement;
    // Kept, not restored — see the discussion composer above.
    expect(after).toBe(box);
  ...
  it('a title being renamed survives a repaint, still open and still typed in', () => {
    ...
    const input = root.querySelector('.board-title-input') as HTMLInputElement;
    expect(input).toBeTruthy();
    typeInto(input, 'Old name, sharper', 3);
    paint({ ...t, updatedAt: NOW + 1 });
    const after = root.querySelector('.board-title-input') as HTMLInputElement;
  ...
  it('keeps a live description AND a half-typed comment across the same repaint', async () => {
    ...
    expect(root.querySelector('.board-detail-body-slot')).toBe(slot);
...
describe('renderTaskDetail — the reorganised panel', () => {
  ...
  const keys = (): string[] =>
    [...root.querySelectorAll('.board-detail-fields dt')].map((dt) => dt.textContent ?? '');
  ...
  const cell = (key: string): HTMLElement | null => {
    const dts = [...root.querySelectorAll('.board-detail-fields dt')];
    const dds = [...root.querySelectorAll<HTMLElement>('.board-detail-fields dd')];
    const i = dts.findIndex((dt) => dt.textContent === key);
  ...
  it('shows the current status once, as a dropdown beside the board’s own mark', () => {
    ...
    expect(status.querySelectorAll('.board-chip')).toHaveLength(0);
    ...
    expect(status.querySelector('.board-status-mark-in-progress')).toBeTruthy();
  ...
  it('has no park block and no park control', () => {
    ...
    expect(root.querySelector('.board-detail-fields')).not.toBeNull(); // control
    expect(root.querySelector('.board-parked-note')).toBeNull();
  ...
  it('orders the panel title → fields → what is waiting → description → tabs', () => {
    ...
    const order = [
      at('.board-detail-title'),
      at('.board-detail-fields'),
      at('.board-decide'),
      at('.board-detail-body-slot'),
      at('.board-detail-tabs'),
    ];
   ...
   * is asserted here and the gaps in `board-decide-css.test.ts`.
   */
  it('groups a decision’s options, and separates them from the free-text box', () => {
    ...
    const card = root.querySelector('.board-decide') as HTMLElement;
    expect(card).toBeTruthy();
    expect(card.querySelector('.board-decide-kicker')?.textContent).toBe(
    ...
    const group = card.querySelector('.board-decide-options') as HTMLElement;
    const opts = [...group.querySelectorAll('.board-decide-option')];
    expect(opts).toHaveLength(2);
    ...
    expect(opts[0]?.querySelector('.board-decide-option-label')?.textContent).toBe('Ship it blue');
    expect(opts[0]?.querySelector('.board-decide-option-detail')?.textContent).toBe(
    ...
    const form = card.querySelector('.board-decide-form') as HTMLElement;
    expect(form.querySelector('.board-decide-form-hint')?.textContent).toBe(
  ...
  it('drops the "or" from the hint when there is nothing to choose between', () => {
    ...
    expect(root.querySelector('.board-decide-options')).toBeNull();
    expect(root.querySelector('.board-decide-form-hint')?.textContent).toBe(
      'Answer in your own words',
    );
  });
  ...
  it('shows no decision card on a task nothing is waiting on', () => {
    ...
    expect(root.querySelector('.board-detail-fields')).toBeTruthy();
  });
  ...
  it('opens on Comments, with Activity present but hidden', () => {
    ...
    const comments = root.querySelector('.board-detail-tabpanel-comments') as HTMLElement;
    const activity = root.querySelector('.board-detail-tabpanel-activity') as HTMLElement;
    expect(comments.classList.contains('hidden')).toBe(false);
    expect(activity.classList.contains('hidden')).toBe(true);
    expect(root.querySelector('.board-detail-tab-comments')?.getAttribute('aria-selected')).toBe(
      'true',
    );
    // The record really is over there rather than nowhere.
    expect(activity.querySelector('.board-detail-body-link')).toBeTruthy();
  });
  ...
  it('keeps the chosen tab across a repaint, and resets it on another task', () => {
    const t = task();
    renderTaskDetail(root, t, handlers(), { loading: false, threads: [] });
    (root.querySelector('.board-detail-tab-activity') as HTMLElement).click();
    const activity = () => root.querySelector('.board-detail-tabpanel-activity') as HTMLElement;
   ...
   * asserted in board-detail-css.test.ts.
   */
  it('parks the tab row at the top when a tab is switched', () => {
    renderTaskDetail(root, task(), handlers(), { loading: false, threads: [] });
    const tabs = root.querySelector('.board-detail-tabs') as HTMLElement;
    const calls: unknown[] = [];
    ...
    expect(root.querySelector('.board-detail-tab-activity')?.getAttribute('aria-selected')).toBe(
      'true',
    );
  });
  ...
  it('offers a share link only when the board wired one up', () => {
    renderTaskDetail(root, task(), handlers());
    expect(root.querySelector('.board-detail-share')).toBeNull();

    root.replaceChildren();
    const onCopyLink = vi.fn();
    const t = task();
    renderTaskDetail(root, t, handlers({ onCopyLink }));
    const share = root.querySelector('.board-detail-share') as HTMLElement;
  ...
  it('names every head action, because each one is a glyph and nothing else', () => {
    ...
    const named = [
      ...root.querySelectorAll<HTMLElement>('.board-detail-head-actions .board-btn'),
    ].map((b) => [b.textContent ?? '', b.getAttribute('aria-label'), b.title]);
  ...
  it('toggles full screen on the container, and keeps it across a repaint', () => {
    ...
    const btn = () => root.querySelector('.board-detail-expand') as HTMLElement;
    expect(btn().getAttribute('aria-label')).toBe('Full screen');
    expect(btn().getAttribute('aria-pressed')).toBe('false');

    btn().click();
    expect(root.classList.contains('board-detail--full')).toBe(true);
    ...
    btn().click();
    expect(root.classList.contains('board-detail--full')).toBe(false);
  });
});
...
describe('the panel’s review queue', () => {
  ...
  const cards = (): string[] =>
    [...root.querySelectorAll<HTMLElement>('.board-decide-card')].map(
      (c) => c.dataset.reviewItemId ?? '',
    );
  const shown = (): HTMLElement | null =>
    root.querySelector<HTMLElement>('.board-decide-card:not(.hidden)');
   ...
   * `reviewReplyRequest` on the Home queue. `board-app` used to build the two
  ...
  describe('the answered record in place, with its persistent Undo', () => {
    ...
    it('renders the record under the item card it answers, marked Answered', () => {
      ...
      expect(card?.querySelector('.board-decide-body')?.textContent).toContain(
        'Blocks the rework.',
      );
      // The record, in place: "Answered by you" for the reader's own answer,
      // rendered markdown-inline.
      const rec = card?.querySelector('.board-detail-answered') as HTMLElement;
      ...
      expect(card?.querySelector('.board-decide-form')).toBeNull();
      expect(card?.querySelectorAll('.board-decide-option')).toHaveLength(0);
      // The kicker says what this card is.
      expect(root.querySelector('.board-decide-kicker')?.textContent).toBe('Answered');
    });
    ...
    it("names the answerer when it wasn't you", () => {
      ...
      expect(shown()?.querySelector('.board-detail-answered')?.textContent).toContain(
        'Answered by Jordan',
      );
    });
    ...
    it('wires the persistent Undo to the thread-answer undo handler', () => {
      ...
      const undo = shown()?.querySelector('.board-detail-undo-answer') as HTMLButtonElement;
    ...
    it('falls back to the tapped option label when a legacy answer has no text', () => {
      ...
      const rec = shown()?.querySelector('.board-detail-answered') as HTMLElement;
  ...
  it('walks several items one at a time, saying which one you are on', () => {
    ...
    expect(root.querySelector('.board-decide-count')?.textContent).toBe('1 of 2');

    const [prev, next] = [...root.querySelectorAll<HTMLButtonElement>('.board-decide-step')];
    expect(prev?.disabled).toBe(true);
    next?.click();
    expect(shown()?.dataset.reviewItemId).toBe('thread:th-a');
    expect(root.querySelector('.board-decide-count')?.textContent).toBe('2 of 2');
  ...
  it('shows no walkthrough chrome at all when there is only one item', () => {
    ...
    expect(root.querySelector('.board-decide-card')).toBeTruthy(); // control
    expect(root.querySelector('.board-decide-walk')).toBeNull();
    expect(root.querySelector('.board-decide-step')).toBeNull();
    expect(root.querySelector('.board-decide-count')?.textContent ?? '').toBe('');
  });
  ...
  it('keeps the walkthrough position across a repaint, and resets it on another task', () => {
    ...
    paint();
    [...root.querySelectorAll<HTMLButtonElement>('.board-decide-step')][1]?.click();
    expect(shown()?.dataset.reviewItemId).toBe('thread:th-a');

    paint({ ...t, status: 'in-progress' });
    expect(
      (root.querySelector('.board-detail-status') as HTMLSelectElement).value, // control
    ).toBe('in-progress');
    expect(shown()?.dataset.reviewItemId).toBe('thread:th-a');
  ...
  it('keeps the SAME item shown when a repaint inserts one ahead of it', () => {
    ...
    expect(shown()?.dataset.reviewItemId).toBe('thread:th-b');
    expect(root.querySelector('.board-decide-count')?.textContent).toBe('3 of 3');
  });
  ...
  it('clamps to the last item when a repaint shrinks the queue under the kept position', () => {
    ...
    const step = () => [...root.querySelectorAll<HTMLButtonElement>('.board-decide-step')][1];
    step()?.click();
    step()?.click();
  ...
  it('answers a thread item as a reply and the task’s decision as a decision', () => {
    ...
    const answerIn = (card: HTMLElement, text: string) => {
      const form = card.querySelector('.board-decide-form') as HTMLFormElement;
      (form.querySelector('textarea') as HTMLTextAreaElement).value = text;
      form.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }));
    };
    const [taskCard, threadCard] = [...root.querySelectorAll<HTMLElement>('.board-decide-card')];
  ...
  it('keeps the queue for the items still open after one is answered', () => {
    ...
    expect(root.querySelector('.board-detail-answer')?.textContent).toContain('Thursday.');
  ...
  it('offers an undo beside the recorded answer, and calls it once', () => {
    ...
    const undo = root.querySelector<HTMLButtonElement>('.board-detail-undo-answer');
    expect(undo).toBeTruthy();
    undo?.click();
  ...
  it('renders no undo when the app offers none, rather than a dead button', () => {
    ...
    expect(root.querySelector('.board-detail-undo-answer')).toBeNull();
    expect(root.querySelector('.board-detail-answer')?.textContent).toContain('Thursday.');
  });
  ...
  it('says why an empty answer did nothing, instead of doing nothing silently', () => {
    ...
    form.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }));
    expect(onAnswer).not.toHaveBeenCalled();
    const note = form.querySelector('.board-form-error');
  ...
  it('shows a rename, a rewrite and a reassignment in the ticket’s own history', () => {
    ...
    const rows = [...root.querySelectorAll('.board-detail-transitions li')].map(
      (li) => li.textContent ?? '',
    );
  ...
  it('names the description, and separates it from the fields and the queue', () => {
    ...
    // `board-detail-css.test.ts`); what belongs here is that the heading exists,
    ...
    const head = root.querySelector('.board-detail-body-head') as HTMLElement;
    expect(head.textContent).toBe('Description');
    const decide = root.querySelector('.board-decide') as HTMLElement;
    const slot = root.querySelector(`.${BODY_LIVE_CLASS}, .board-detail-body-slot`) as HTMLElement;
  ...
  it('marks the body while the panel is open, and unmarks it on close', () => {
    ...
    expect(root.querySelector('.board-detail-panel')).toBeNull(); // control
  });
  ...
  it('offers the board’s goals, keeps the task’s own, and commits a move', () => {
    ...
    const sel = root.querySelector('.board-detail-goal') as HTMLSelectElement;
  ...
  it('keeps a separate answer draft per item', () => {
    ...
    const keys = [...root.querySelectorAll<HTMLTextAreaElement>('.board-decide-form textarea')].map(
      (ta) => ta.dataset.keep,
    );
...
describe('the description slot the editor mounts into', () => {
  ...
  const slot = () => root.querySelector('.board-detail-body-slot') as HTMLElement | null;
  ...
  it('holds the description, and says which task it belongs to', () => {
    ...
    expect(s?.querySelector('.board-detail-body strong')?.textContent).toBe('read');
  });
  ...
  it('keeps the very same node across a repaint once the editor owns it', () => {
    ...
    expect(root.querySelector('.board-detail-title')?.textContent).toBe('Renamed');
  });
  ...
  it('the link out is a second way in, not the way to edit', () => {
    ...
    renderTaskDetail(root, task(), detailHandlers());
    expect(root.querySelector('.board-detail-body-link a')?.textContent).toBe(
