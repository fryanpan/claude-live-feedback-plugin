 ...
 * element is swapped and no box is rebuilt (`board-render.test.ts`, "edits the
...
function titleRow(state: 'resting' | 'editing') {
  const row = attach('board-task-row');
  const cell = attach(
    state === 'editing' ? 'board-task-title board-title-editing' : 'board-task-title',
    {
      parent: row,
    },
  );
  const text = attach('board-task-title-text', {
    tag: 'span',
    parent: cell,
...
describe('the row title, resting and edited', () => {
  ...
  it('keeps one grid track per row child, with the flexible one under the title', () => {
    ...
    // list in `board-render.test.ts` are one fact in two files: six children,
