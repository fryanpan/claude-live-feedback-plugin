...
import { WALK_HANDOFF_DEADLINE_MS, createBoardDeepLinks } from '../src/board/board-deep-links.ts';
import { type ReviewQueue, reviewQueue } from '../src/board/board-review-model.ts';
 ...
 */
function deepLinks(over: Partial<Parameters<typeof createBoardDeepLinks>[0]> = {}, search = '') {
  const state = boardState({ pane: 'home', nav: 'home' });
  ...
  const api = createBoardDeepLinks({
    state,
...
describe('createBoardDeepLinks', () => {
  ...
  it('closes a panel whose ?task= never appeared, and says the link is outdated', () => {
    const state = boardState({ detailTaskId: 't-gone', pane: 'board' });
    const d = deepLinks({
      state,
      bootLoc: {
        nav: 'tasks',
