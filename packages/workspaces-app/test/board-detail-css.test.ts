 ...
 *     which costs the two assertions that the panel SPENDS `--board-detail-w`
 *     in the modal and `--board-detail-pane-w` in the split pane;
 *   · `padding-right` carrying a percentage — also '' — which costs the
 *     assertion that `.board-main`'s reservation exists and is clamped. The
...
beforeEach(() => {
  ...
  // shells.ts loads board.css BEFORE styles.css, and says why: the board block
...
function tokenPx(name: string, viewportWidth: number, viewportHeight = 900): number {
  ...
  probe.textContent = `.board-detail-token-probe { max-width: var(${name}); }`;
  document.head.appendChild(probe);
  const raw = styleOf(attach('board-detail-token-probe')).maxWidth;
  probe.remove();
  return evalPx(raw);
}

/** True once the split-pane block applies: `.board-detail` is only given
...
function splitApplies(viewportWidth: number): boolean {
  ...
  return styleOf(attach('board-detail')).justifyContent === 'flex-end';
}
...
function splitPanel(vw: number): CSSStyleDeclaration {
  ...
  document.body.className = 'board-detail-open';
  return styleOf(attach('board-detail-panel', { parent: attach('board-detail') }));
}
...
describe('the task panel is as wide as it was asked to be', () => {
  it('resolves its width from ONE expression: 1100 by default, floored at 900', () => {
    ...
    expect(tokenPx('--board-detail-w', 6000)).toBe(1100);
    expect(tokenPx('--board-detail-w', 400)).toBe(900);
    const at1600 = tokenPx('--board-detail-w', 1600);
    const at1700 = tokenPx('--board-detail-w', 1700);
  ...
  it('positive control: an undefined token resolves to nothing at all', () => {
    ...
    expect(Number.isNaN(tokenPx('--board-detail-not-a-token', 1600))).toBe(true);
    expect(Number.isFinite(tokenPx('--board-detail-w', 1600))).toBe(true);
  });

  it('caps the pane by what the board keeps, from the same expression', () => {
    // The split resolves the width through `--board-detail-pane-w`, which is the
    ...
    for (const vw of [1661, 1920, 2400, 4000]) {
      const keep = tokenPx('--board-keep', vw);
      expect(tokenPx('--board-detail-pane-w', vw), `${vw}px`).toBe(
        Math.min(tokenPx('--board-detail-w', vw), vw - keep),
      );
    }
    // NOT asserted here, and asserted in the text version this replaces: that
    // `.board-detail-panel` SPENDS these tokens in its `width`. happy-dom
  ...
  it('reflows the board out from under the panel, and gives it back at full screen', () => {
    ...
    document.body.className = 'board-detail-open board-detail-full';
    // At full screen the panel covers the board, so the reservation is given
    // back — the readable half of the pair.
    expect(Number.parseFloat(styleOf(attach('board-main')).paddingRight)).toBe(0);
    ...
    // `.board-main`). happy-dom returns '' for a padding carrying a percentage,
...
describe('the split pane starts where both columns fit', () => {
  ...
  it('opens no lower than panel floor + board floor + chrome', () => {
    const breakpoint = splitBreakpoint();
    const panelMin = tokenPx('--board-detail-w', 400);
    const boardMin = tokenPx('--board-min', breakpoint);
    const chrome = tokenPx('--board-chrome', breakpoint);
  ...
  it('caps the pane so the board keeps its floor at every width above it', () => {
    ...
    expect(tokenPx('--board-detail-pane-w', breakpoint)).toBeLessThanOrEqual(
      breakpoint - tokenPx('--board-keep', breakpoint),
    );
  });

  it('states the board’s keep as its floor plus the chrome, in one place', () => {
    // `--board-keep` is what both the pane cap and the panel's own clamp
    ...
    for (const vw of [1661, 1920, 2400]) {
      expect(tokenPx('--board-keep', vw), `${vw}px`).toBe(
        tokenPx('--board-min', vw) + tokenPx('--board-chrome', vw),
      );
    }
  });
});
 ...
 * straight off `#board-root`'s computed `max-width`. One input is not: the
 ...
 * falsify them, 2026-09-03: deleting the `#board-root` growth, deleting the pane
 * cap, and moving `--board-min`, `--board-page-w` and `--board-chrome`
 ...
 * the chrome term of `--board-keep`, which is why that constant being
...
describe('the board keeps its floor in rendered pixels, not just in tokens', () => {
   ...
   *  `--board-chrome`, so the model cannot drift from the stylesheet. */
  const gutters = (vw: number) =>
    tokenPx('--board-chrome', vw) -
    tokenPx('--board-inset', vw) -
    tokenPx('--board-detail-gap', vw);

  /**
   * What `#board-root` is capped at while the panel is open, READ FROM THE
  ...
  const openCapAt = (vw: number): number => {
    setViewport({ width: vw, height: 900 });
    document.body.className = 'board-detail-open';
    const root = attach('', { attrs: { id: 'board-root' } });
   ...
   * container's open-state cap, `.board-main` inside its gutters, and the
   ...
   * anyway, because it is expressed in `.board-main`'s own coordinates and so
  ...
  const boardAt = (vw: number, sb = 0): number => {
    const inset = tokenPx('--board-inset', vw);
    const boardMin = tokenPx('--board-min', vw);
    const rootW = Math.min(openCapAt(vw), vw - sb);
    const mainW = rootW - gutters(vw);
    const want = tokenPx('--board-detail-pane-w', vw) + tokenPx('--board-detail-gap', vw);
  ...
  it('reproduces six measured pixel counts before asserting anything', () => {
    ...
    // And the one recorded in `--board-min`'s own comment, from before a
    ...
    //
    // With the container cap fixed and keep = 420 + 234, `#board` came
  ...
  it('reads every number from the cascade, so the comparisons below are not vacuous', () => {
    for (const [name, n] of [
      ['--board-min', tokenPx('--board-min', 1920)],
      ['--board-inset', tokenPx('--board-inset', 1920)],
      ['--board-chrome', tokenPx('--board-chrome', 1920)],
      ['--board-page-w', tokenPx('--board-page-w', 1920)],
      ['--board-detail-gap', tokenPx('--board-detail-gap', 1920)],
      ['--board-detail-w', tokenPx('--board-detail-w', 1920)],
      ['--board-detail-pane-w', tokenPx('--board-detail-pane-w', 1920)],
    ...
    expect(openCapAt(1920)).toBeGreaterThan(tokenPx('--board-page-w', 1920));
    expect(openCapAt(1920)).toBeCloseTo(
      tokenPx('--board-page-w', 1920) +
        tokenPx('--board-detail-pane-w', 1920) +
        tokenPx('--board-detail-gap', 1920),
      1,
    );
    // …and below the breakpoint it is the page's own cap, unchanged.
    expect(openCapAt(1400)).toBe(tokenPx('--board-page-w', 1400));
    ...
    expect(evalPx(splitPanel(1920).maxWidth, 1920)).toBe(1920 - tokenPx('--board-keep', 1920));
  });
    ...
    for (const vw of [1661, 1720, 1920, 2400, 4000]) {
      ...
      for (const sb of scrollbars) {
        ...
        const panelW = Math.min(
          tokenPx('--board-detail-pane-w', vw),
  ...
  it('honours --board-min at 1661, 1920 and 2400', () => {
    for (const vw of [1661, 1920, 2400]) {
      const boardMin = tokenPx('--board-min', vw);
      for (const sb of scrollbars) {
  ...
  it('never lets the board exceed what the closed page would have given it', () => {
    const closedBoard =
      tokenPx('--board-page-w', 4000) -
      tokenPx('--board-chrome', 4000) +
      tokenPx('--board-detail-gap', 4000);
...
describe('the tab row is reachable where it appears', () => {
  it('docks under the sticky head rather than sliding beneath it', () => {
    setViewport(IPAD);
    const tabs = styleOf(attach('board-detail-tabs'));
    expect(tabs.position).toBe('sticky');
    // Opaque, or the comments scroll through the labels.
    expect(tabs.backgroundColor).not.toBe('');
    expect(styleOf(attach('board-detail-tabs')).backgroundColor).toBe(
      styleOf(attach('board-detail-panel')).backgroundColor,
    );
  ...
  it('takes its offset from the head’s MEASURED height, with a usable fallback', () => {
    ...
    // and the dock moves.
    setViewport(IPAD);
    const fallback = Number.parseFloat(styleOf(attach('board-detail-tabs')).top);
    expect(fallback).toBeGreaterThan(0);
    document.documentElement.style.setProperty('--board-detail-head-h', '137px');
    expect(styleOf(attach('board-detail-tabs')).top).toBe('137px');
    document.documentElement.style.removeProperty('--board-detail-head-h');
    expect(Number.parseFloat(styleOf(attach('board-detail-tabs')).top)).toBe(fallback);
  });
  ...
  it('lands a switch under the head, not under the scrollport’s own top', () => {
    ...
    for (const published of [null, '137px']) {
      if (published) document.documentElement.style.setProperty('--board-detail-head-h', published);
      const tabs = styleOf(attach('board-detail-tabs'));
...
describe('the panel’s fields and headings', () => {
  ...
  function fields(viewport: { width: number; height: number }) {
    ...
    return attach('board-detail-fields', { parent: attach('board-detail-panel') });
  }
  ...
  it('gives the goal its own line, because a goal title is free text', () => {
    ...
    const full = attach('board-detail-field board-detail-field--full', { parent: grid });
    ...
    const first = attach('board-detail-field', { parent: grid2 });
    attach('board-detail-field', { parent: grid2 });
  ...
  it('holds the description to a readable measure when the panel goes wide', () => {
    ...
    setViewport(IPAD);
    const slot = styleOf(attach('board-detail-body-slot'));
  ...
  it('draws all four values as one kind of control at one height', () => {
    setViewport(IPAD);
    const cell = attach('board-detail-field board-detail-field-v');
    for (const [cls, tag] of [
      ['board-detail-select', 'select'],
      ['board-detail-input', 'input'],
    ] as const) {
    ...
    const pill = attach('board-assignee-btn', { tag: 'button', parent: cell });
    ...
    expect(
      Number.parseFloat(styleOf(attach('board-assignee-btn', { tag: 'button' })).minHeight),
    ).toBeGreaterThan(36);
  });
  ...
  it('puts the status mark and its dropdown on one line', () => {
    ...
    setViewport(IPAD);
    const cell = attach('board-detail-field board-detail-field-v');
    const ctl = attach('board-detail-statusctl', { parent: cell });
    ...
    expect(styleOf(attach('board-status-mark', { parent: ctl })).flex).toBe('0 0 auto');
    expect(
      Number.parseFloat(
        styleOf(attach('board-detail-status', { tag: 'select', parent: ctl })).minWidth,
      ),
    ).toBe(0);
    // The chip row is GONE, not merely unused: an element carrying its class
    // reads exactly like one carrying no class at all.
    expect(styleOf(attach('board-detail-statuses')).display).toBe('block');
    expect(styleOf(attach('board-detail-statuses')).gap).toBe('');
  });
  ...
  it('separates the Description heading from the fields above it', () => {
    ...
    setViewport(IPAD);
    const panel = attach('board-detail-panel');
    const head = attach('board-detail-body-head', { tag: 'h3', parent: panel });
    ...
    const after = attach('board-detail-body-slot', { parent: panel });
    expect(styleOf(after).marginTop).toBe('0px');
    expect(Number.parseFloat(styleOf(attach('board-detail-body-slot')).marginTop)).toBeGreaterThan(
      0,
    );
  });
  ...
  it('keeps the live description on the panel’s spine, border included', () => {
    ...
    setViewport(IPAD);
    const live = styleOf(attach('board-detail-body-live'));
 ...
 * content at any width; `board-mic-dock-css.test.ts` holds that guarantee, and
...
describe('every control in the queue clears the touch-target floor', () => {
  it('keeps every control in the queue at the 36px floor', () => {
    ...
    setViewport(IPAD);
    const step = styleOf(attach('board-decide-step', { tag: 'button' }));
...
describe('the record block still says it opens', () => {
  it('keeps the summary a list-item, so Chrome draws its disclosure triangle', () => {
    ...
    setViewport(IPAD);
    const label = styleOf(
      attach('board-detail-quote-label', {
        tag: 'summary',
        parent: attach('', { tag: 'details' }),
      }),
    );
...
describe('the threading UI left no rules behind', () => {
  ...
  it('has no rule for a control the panel no longer renders', () => {
    ...
    setViewport(IPAD);
    const host = attach('board-detail-panel');
    const bare = shapeOf(attach('', { parent: host }));
    for (const sel of [
      'board-comment-reply',
      'board-comment-anchor',
      'board-comment-status',
      'board-comment-resolved',
      'board-comment-needs-you',
      'board-composer-target',
      'board-composer-target-label',
      'board-composer-switch',
      'board-detail-ask',
      'board-detail-ask-kicker',
      'board-detail-ask-form',
      ...
      // card's one markdown body (`.board-decide-body`), so the rule it was
      // written against is dead CSS of exactly the kind this sweep exists to
      // catch.
      'board-detail-ask-why',
      ...
      'board-walk-why',
      'board-walk-ctx',
      'board-walk-asked-line',
      'board-walk-lookfor-text',
      'board-walk-review-detail',
      'board-review-row-why',
      'board-decide-why',
      'board-decide-detail',
      'board-decide-lookfor',
      ...
      // chips could mark. `.board-chip-todo/-in-progress/-done` are NOT here —
    ...
    expect(shapeOf(attach('board-comment', { parent: host }))).not.toEqual(bare);
    // …and the surviving status colors really survive: the <select> at
    // board-render's status control still emits `board-chip-<status>`.
    expect(shapeOf(attach('board-chip-done', { parent: host }))).not.toEqual(bare);
    // NOT swept here, and swept in the text version: `.board-status-chip:hover`
