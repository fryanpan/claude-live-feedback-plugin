...
import { createHubDiscussion, goalBodyDocId } from '../src/hub/hub-discussion.ts';
 ...
 */
function discussion(over: Partial<Parameters<typeof createHubDiscussion>[0]> = {}) {
  const state = hubState();
  const renderDetail = vi.fn();
  const api = createHubDiscussion({
    state,
