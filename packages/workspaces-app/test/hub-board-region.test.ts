...
import type { HubActions } from '../src/hub/hub-actions.ts';
import { createHubBoardRegion } from '../src/hub/hub-board-region.ts';
 ...
 * the wrong thread.
 */
function region(over: Partial<Parameters<typeof createHubBoardRegion>[0]> = {}) {
  const el = mountShell();
  ...
  const setNav = vi.fn();
  const api = createHubBoardRegion({
    state,
    user: { name: 'Bryan' },
    el,
    actions,
...
describe('createHubBoardRegion', () => {
  ...
  it('swaps the restore list in beside the island, never inside it', () => {
    // `#hub-board` is the island's host for the life of the page, so the
    // archived view has to be a sibling container.
    const r = region();
    r.setShowArchived(true);
    expect(r.el('hub-board').classList.contains('hidden')).toBe(true);
    expect(r.el('hub-archived').classList.contains('hidden')).toBe(false);
    expect(r.el('hub-archived').children.length).toBeGreaterThan(0);
  ...
  it('hides everything that belongs to the task list when Activity is up', () => {
    ...
    for (const id of ['hub-quick', 'hub-decisions', 'hub-archived']) {
      expect(r.el(id).classList.contains('hub-hidden-by-view')).toBe(true);
    }
    expect(r.el('hub-activity').classList.contains('hidden')).toBe(false);
    expect(r.el('hub-board').classList.contains('hidden')).toBe(true);
  });
  ...
  it('brings them all back when the reader leaves Activity', () => {
    ...
    r.renderActivityRegion();
    for (const id of ['hub-quick', 'hub-decisions', 'hub-archived']) {
      expect(r.el(id).classList.contains('hub-hidden-by-view')).toBe(false);
    }
    expect(r.el('hub-activity').classList.contains('hidden')).toBe(true);
  });
});
