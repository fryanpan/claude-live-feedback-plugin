 ...
 * copy emitted `board-detail-options` / `board-detail-option` /
 * `board-detail-option-label` / `board-detail-option-detail` and the stylesheet had
 * **no rule for any of them** — so the browser's defaults stacked the options
 * edge to edge and jammed the answer box against them. `board-render.test.ts`
 * asserts the structure the rules hang off; this asserts that the rules reach
 * it.
 *
 * "Reach it" is the word that changed. This file used to regex `board.css` for a
...
const CARD: ReadonlyArray<readonly [string, keyof CSSStyleDeclaration, string]> = [
  ['board-decide', 'padding', '16px'],
  ['board-decide-head', 'display', 'flex'],
  ['board-decide-kicker', 'fontWeight', '700'],
  ['board-decide-card-head', 'alignItems', 'baseline'],
  ['board-decide-k', 'padding', '2px 7px'],
  ['board-decide-headline', 'lineHeight', '1.4'],
  ['board-decide-body', 'lineHeight', '1.5'],
  ['board-decide-meta', 'fontSize', '12px'],
  ['board-decide-walk', 'display', 'flex'],
  ['board-decide-step', 'display', 'inline-flex'],
  ['board-decide-count', 'whiteSpace', 'nowrap'],
  ['board-decide-options', 'flexDirection', 'column'],
  ['board-decide-option', 'padding', '12px'],
  ['board-decide-option-label', 'fontWeight', '600'],
  ['board-decide-option-detail', 'lineHeight', '1.4'],
  ['board-decide-form', 'flexDirection', 'column'],
  ['board-decide-form-hint', 'fontSize', '13px'],
];
...
describe('the decision card has the spacing it was reported for missing', () => {
  it('styles every class the card emits — the reported bug was that none of them were', () => {
    ...
    // browser's defaults.
    const missing = el('board-decide-nothing-owns-this');
  ...
  it('lets the blurb run to as many lines as it needs', () => {
    ...
    for (const cls of ['board-decide-headline', 'board-decide-body']) {
      const r = el(cls);
  ...
  it('lets the asked-by meta wrap — it is a sentence now, and 430px is real', () => {
    ...
    expect(el('board-walk-wait', 'span').whiteSpace).not.toBe('nowrap');
    expect(el('board-decide-meta').whiteSpace).not.toBe('nowrap');
    ...
    expect(el('board-decide-count', 'span').whiteSpace).toBe('nowrap');
  });
  ...
  it('puts the walkthrough beside the kicker without moving it', () => {
    ...
    const head = el('board-decide-head');
    ...
    expect(Number.parseFloat(el('board-decide-step', 'button').minHeight)).toBeGreaterThanOrEqual(
      36,
    );
    // Only one card is on screen; the rest are hidden rather than unbuilt.
    expect(el('board-decide-card hidden').display).toBe('none');
    expect(el('board-decide-card').display).not.toBe('none'); // control
  });
  ...
  it('puts real space between the answer buttons', () => {
    ...
    const opts = el('board-decide-options');
  ...
  it('gives each option padding, and its detail its own line', () => {
    ...
    const opt = el('board-decide-option', 'button');
  ...
  it('separates the free-text box from the options above it, and only then', () => {
    ...
    // this is the adjacent-sibling rule and not `.board-decide-form` itself.
    const card = attach('board-decide-card');
    attach('board-decide-options', { parent: card });
    const after = styleOf(attach('board-decide-form', { parent: card }));
    ...
    const alone = styleOf(attach('board-decide-form', { parent: attach('board-decide-card') }));
  ...
  it('is written to be reusable by the Home card, not scoped to the panel', () => {
    ...
    const inPanel = attach('board-decide', {
      parent: attach('board-detail-panel', { parent: attach('board-detail') }),
    });
    const onHome = attach('board-decide', { parent: attach('board-home') });
