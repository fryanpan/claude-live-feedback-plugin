...
import { parseBoardLocation } from '../src/hub/board-url.ts';
import { createHubProjection, initialHubState } from '../src/hub/hub-projection.ts';
...
describe('createHubProjection', () => {
  ...
  it('renames the board in the header and the tab without a reload', () => {
    ...
    p.readProjection();
    expect(el('hub-ws-name-text').textContent).toBe('harbor-relay');
    expect(document.title).toContain('harbor-relay');

    p.ws.set('name', 'harbor-relay-september');
    p.readProjection();
    expect(el('hub-ws-name-text').textContent).toBe('harbor-relay-september');
  ...
  it('raises the retired badge when the projection says the board is retired', () => {
    ...
    expect(el('hub-retired-badge').classList.contains('hidden')).toBe(false);
  });
  ...
  it('leaves state.info alone until the room has synced an id', () => {
    ...
    const state = hubState({ info: { id: 'w-1', name: 'from-rest', goals: [], createdAt: 1 } });
...
describe('initialHubState', () => {
  const stateFor = (pathname: string, search = '') =>
    initialHubState(parseBoardLocation(pathname, search));
