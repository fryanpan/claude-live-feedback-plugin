 ...
 * thumb target. This file used to read `styles.css` and `board.css` as text and
...
/** The box the buttons replaced: `.board-quick-input { min-height: 40px }`. */
...
function stylingOf(classes: string, tag = 'div'): string[] {
  const parent = attach('board-quick');
  const subject = styleOf(attach(classes, { tag, parent }));
  const control = styleOf(attach('board-none-of-the-above', { tag, parent }));
  const out: string[] = [];
...
function quickButton(viewport: { width: number; height: number }, classes = 'board-btn') {
  setViewport(viewport);
  const row = attach('board-quick-actions', { parent: attach('board-quick') });
...
describe('the quick-add box is gone from the stylesheet too', () => {
  it('leaves the box, its form, its mic and its submit completely unstyled', () => {
    ...
    for (const [sel, tag] of [
      ['board-quick-input', 'input'],
      ['board-quick-form', 'form'],
      ['board-quick-mic', 'button'],
      ['board-quick-submit', 'button'],
    ] as const) {
  ...
  it('positive control: the slot they lived in is still styled', () => {
    ...
    setViewport(IPAD);
    expect(stylingOf('board-quick-actions').join(' ')).toContain('display: flex');
    expect(styleOf(attach('board-quick')).marginBottom).not.toBe('');
  });
});
...
describe('the buttons sit in the slot the box had', () => {
  it('is one wrapping row', () => {
    setViewport(IPAD);
    const row = styleOf(attach('board-quick-actions', { parent: attach('board-quick') }));
  ...
  it('costs no more height than the box at 1180×820', () => {
    ...
    expect(px(styleOf(attach('board-quick')).marginBottom)).toBeLessThanOrEqual(10);
  });
  ...
  it('is a 44px thumb target at 430px', () => {
    ...
    // a bare `.board-btn` elsewhere on the phone keeps the smaller size, so a
    // rule scoped to the wrong ancestor fails here rather than passing on the
    // base value.
    setViewport(PHONE);
    expect(px(styleOf(attach('board-btn', { tag: 'button' })).minHeight)).toBeLessThan(44);
  });
  ...
  it('sizes EVERY quick-action glyph as a box, like every other mic', () => {
    ...
    for (const button of ['board-quick-new', 'board-huddle-start', 'board-conversation-start']) {
    ...
    const stray = quickButton(IPAD, 'board-nonexistent-button');
...
describe('an unnamed task reads as a stand-in, not a title', () => {
  it('mutes the placeholder', () => {
    ...
    setViewport(IPAD);
    const titled = styleOf(attach('board-detail-title'));
    const placeholder = styleOf(attach('board-detail-title board-detail-title-placeholder'));
