 ...
 * grepping `hub.css` for `var(--kb-bottom)`, these DRIVE it: `--kb-bottom` is
...
beforeEach(() => {
  ...
  // (`body.hub-body { --hub-bottom-bar: 58px }` in the ≤900 block among them),
...
function overlayBottom(vp: { width: number; height: number }): string {
  setViewport(vp);
  return styleOf(attach('hub-detail')).bottom;
}
...
describe('the task panel clears the on-screen keyboard', () => {
  ...
  it('caps the panel against the overlay it sits in, not against the raw viewport', () => {
    ...
    setViewport(IPAD);
    const tall = styleOf(attach('hub-detail-panel', { parent: attach('hub-detail') })).maxHeight;
    setViewport({ width: IPAD.width, height: 600 });
    const short = styleOf(attach('hub-detail-panel', { parent: attach('hub-detail') })).maxHeight;
  ...
  it('hands the sheet edge to the phone sheet at 430px, where the bottom bar lives', () => {
    ...
    expect(styleOf(attach('hub-detail-panel', { parent: attach('hub-detail') })).maxHeight).toBe(
      '100%',
    ); // a full-height sheet, not the 92vh card
    // Control: the desktop values really are different, so the two reads above
    // are the phone block talking.
    setViewport(IPAD);
    expect(styleOf(attach('hub-detail')).padding).not.toBe('0px');
  });
});
