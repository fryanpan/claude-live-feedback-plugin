...
beforeEach(() => {
  ...
  // shells.ts loads hub.css BEFORE styles.css, and the order is load-bearing
...
function noteBody(viewport: { width: number; height: number }, folded: boolean) {
  ...
  return attach(folded ? 'hub-note-body is-folded' : 'hub-note-body');
}
...
describe('the task Activity feed is height-frugal at the tablet tier', () => {
  it('a feed row and a note body take no fixed height', () => {
    ...
    expect(styleOf(attach('hub-note-body')).overflowWrap).toBe('anywhere');
    expect(styleOf(attach('hub-hist-row')).display).not.toBe('');
  });
  ...
  it('the kind label is a small caps token beside the agent and the age', () => {
    ...
    expect(size).toBeLessThan(Number.parseFloat(styleOf(attach('hub-note-body')).fontSize));
  });
  ...
  it('a status token is tinted so a milestone reads apart from a routine turn', () => {
    ...
    const kindIn = (rowClass: string) =>
      styleOf(attach('hub-note-kind', { parent: attach(`hub-hist-row ${rowClass}`) })).color;
    const status = kindIn('hub-hist-row-status');
    expect(status).not.toBe('');
    // Three different rows, three different marks: a status is not a denial,
    // and neither wears the plain feed colour.
    expect(status).not.toBe(kindIn('hub-hist-row-denial'));
    expect(status).not.toBe(styleOf(attach('hub-note-kind')).color);
  });

  it('the marked phrase in the feed wears the active thread-range treatment', () => {
    setViewport(IPAD);
    const mark = styleOf(
      attach('thread-range', { parent: attach('hub-detail-transitions') }),
    ).backgroundColor;
...
describe('the task Activity feed is thumb-sized on the phone tier', () => {
  it('the fold toggle and the pill grow to 44px at 430px', () => {
    setViewport(PHONE);
    expect(Number.parseFloat(styleOf(attach('hub-note-more', { tag: 'button' })).minHeight)).toBe(
      44,
    );
    const pill = styleOf(attach('hub-hist-pill', { tag: 'button' }));
    ...
    setViewport(IPAD);
    expect(
      Number.parseFloat(styleOf(attach('hub-hist-pill', { tag: 'button' })).minHeight) || 0,
    ).toBeLessThan(44);
  });

  it('negative control: a selector the sheet does not have reads as unstyled', () => {
    setViewport(PHONE);
    const nothing = styleOf(attach('hub-hist-nothing'));
