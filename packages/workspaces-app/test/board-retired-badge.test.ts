...
import { renderWorkspaceIdentity } from '../src/board/board-render.ts';
...
describe('renderWorkspaceIdentity', () => {
  ...
  beforeEach(() => {
    document.body.innerHTML =
      '<span class="board-ws-name"><span id="n" class="board-ws-name-text"></span>' +
      '<span id="b" class="board-retired-badge hidden">Retired</span></span>';
...
function paintIdentity(viewport: { width: number; height: number }) {
  ...
  return {
    row: document.querySelector('.board-ws-name') as HTMLElement,
    name,
    badge,
  };
}
