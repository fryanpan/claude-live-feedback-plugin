...
describe('renderRelatedLinks', () => {
  ...
  it('builds a heading and one link per entry, canonical workspace href first', () => {
    ...
    expect(el?.querySelector('.board-related-links-k')?.textContent).toBe('Related Links');
    const anchors = [...(el?.querySelectorAll<HTMLAnchorElement>('.board-related-link') ?? [])];
  ...
  it('carries the held note on the marked entry only', () => {
    ...
    expect(items[0]?.querySelector('.board-related-link-held')?.textContent).toContain(
      'held until',
    );
    expect(items[1]?.querySelector('.board-related-link-held')).toBeNull();
  });
});
...
describe('the Related Links section on the panel', () => {
  it('renders below the fields row, and not at all with no doc ties', () => {
    ...
    // replaces it.
    expect(
      [...root.querySelectorAll('.board-detail-field-k')].map((k) => k.textContent),
  ...
  it('shows the doc TITLE when the shared link-title cache knows it', () => {
    ...
    expect(root.querySelector('.board-related-link')?.textContent).toBe('Sprint plan');
  });
  ...
  it('carries the plan-hold mark, and never a staleness mark (dropped by design)', () => {
    ...
    expect(root.querySelector('.board-related-link-held')?.textContent).toContain('held until');
    // A possiblyStale row draws NO "plan edited since filed" mark — the flag
    // gives the reader nothing to act on (Bryan, 2026-08-31).
    expect(root.querySelector('.board-related-link-stale')).toBeNull();
    ...
    expect(root.querySelector('.board-related-link')).not.toBeNull(); // control
    expect(root.querySelector('.board-related-link-held')).toBeNull();
  });
});
