 ...
 *
 * This file used to read `board.css` as text and brace-walk its own copy of the
...
describe('archive CSS', () => {
  it('the restore row wraps, so the button drops rather than crushing the title', () => {
    setViewport(PHONE);
    const row = styleOf(attach('board-archived-row', { tag: 'li' }));
  ...
  it('the archived title can shrink — min-width:0 is what allows the ellipsis', () => {
    setViewport(PHONE);
    const title = styleOf(attach('board-archived-title', { tag: 'span' }));
  ...
  it('every added control clears the 36px thumb floor', () => {
    for (const sel of ['board-archived-restore', 'board-toast-action']) {
    ...
    setViewport(PHONE);
    expect(styleOf(attach('board-archived-not-a-control', { tag: 'button' })).minHeight).toBe('');
  });

  it('the toast lays its action out beside the text rather than under it', () => {
    setViewport(PHONE);
    const toast = styleOf(attach('board-toast'));
    ...
    expect(styleOf(attach('board-toast hidden')).display).toBe('none');
  });
  ...
  it('the board foot line is small and quiet, and a 44px thumb target on the phone tier', () => {
    ...
    setViewport(IPAD);
    const foot = styleOf(attach('board-foot'));
    expect(foot.display).toBe('flex'); // control: the foot is a styled row
    expect(foot.justifyContent).not.toBe('flex-end');
    expect(styleOf(attach('board-foot-archived', { tag: 'button' })).fontSize).toBe('12.5px');
    // The target grows on the phone tier; the type does not.
    expect(floorOf(PHONE, 'board-foot-archived')).toBeGreaterThanOrEqual(44);
    expect(floorOf(IPAD, 'board-foot-archived')).toBeLessThan(44);
    ...
    setViewport(PHONE);
    const quick = attach('board-quick-actions');
    expect(
      Number.parseFloat(styleOf(attach('board-btn', { tag: 'button', parent: quick })).minHeight),
    ...
    expect(styleOf(attach('board-meta')).display).toBe('block');
  });
  ...
  it('the archived note is its own quiet boxed panel, not another row in the list', () => {
    ...
    setViewport(PHONE);
    const note = styleOf(attach('board-archived-note'));
