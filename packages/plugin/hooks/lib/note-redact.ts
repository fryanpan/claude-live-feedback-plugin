/**
 * Note redaction — the reduction half of the plugin's Stop and
 * PermissionDenied hooks. `agent-notes.ts` decides WHETHER to post and
 * where; this file decides what the text may say.
 *
 * The one rule the whole file serves: reduce, never forward. A closing
 * message keeps its lines and its markdown but every word on every line —
 * fenced code included — goes through the same reduction: URLs and host
 * paths, token prefixes, `Bearer` values and emails come out as markers or
 * a bare file name; behind those, a long opaque run, a base64 blob, and the
 * value of any secret-named key or assignment come out as `[redacted]`,
 * because the named shapes only ever cover the providers somebody listed. A
 * denied command becomes its command word plus a subcommand or flag, with
 ...
 * reduction happens.
 *
 * Three entry points, and everything else here is private to them:
 * `fullNote` (a whole closing message), `oneLine` (the same message reduced
 * to one line — the Home pane's derivation, kept as the reference form) and
 * `commandShape` (a denied Bash command's shape). Like its parent, this
 * module imports nothing from the monorepo: the hooks run from the
 * INSTALLED plugin, where `packages/plugin` is all there is.
 */
