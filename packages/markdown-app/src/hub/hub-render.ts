/**
 * The hub's shell chrome (plan §3.9): the topbar, the lead-agent strip, the
 ...
 * `hub-detail-render.ts` (the task panel), `hub-discussion-render.ts` (the
 * comment stream) and `hub-review-render.ts` (the review queue and its
...
import { type ReviewQueue, reviewBannerText } from './hub-review-model.ts';
