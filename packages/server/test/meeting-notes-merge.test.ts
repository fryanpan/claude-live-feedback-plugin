...
describe('mergeNotesSection — regrouping a topic a person has typed in', () => {
  const HUMAN = 'MY OWN note, in my words';

  /**
   * Four bullets the note-taker wrote, and the person's own line at the top
   * of the same list. This is the section a regroup arrives into, and the
   * one shape both holes below need: their line and the agent's bullets in
   * ONE list, so a heading can only land between two items of it.
   */
  const typedInto = (): {
    ydoc: Y.Doc;
    own: NotesOwnership;
    ...
    const human = typeBullet(ydoc, 0, HUMAN);
    return { ydoc, own, basedOn: readNotesSection(ydoc, HEADING, own)!.items, human };
  };

  /** The line still in the doc, as the element that holds it. */
  const humanItems = (ydoc: Y.Doc): NoteItem[] =>
    sectionItems(ydoc).filter((i) => i.md.includes('MY OWN note'));

  it('never leaves two of their line when the regroup nests a copy of it', () => {
    const { ydoc, own, basedOn, human } = typedInto();
    // The composer keeps their line where it is AND restates it inside a
    // group of its own points. Both copies read as the agent's new writing
    // to the ledger — a nested line is not an item at all, so nothing in the
    // plan ever compares it to anything — which is how the second one used
    // to land.
    const regrouped = [
      '## Meeting notes',
      '',
    ...
    const merged = mergeNotesSection(ydoc, regrouped, HEADING, { ownership: own, basedOn });

    expect(merged.ok).toBe(true);
    const md = markdownOf(ydoc);
    expect(md.split(HUMAN)).toHaveLength(2); // their line, exactly once
    // Their words, in their element: the copy went, not the original.
    expect(humanItems(ydoc)).toHaveLength(1);
    expect(humanItems(ydoc)[0]!.el).toBe(human);
    // And the group it was nested in survived it, minus the copy.
    expect(md).toContain('What the dialog gets wrong');
    expect(md).toContain('Agent bullet one');
    expect(md).toContain('Agent bullet two');
    expect(md).toContain('Owner and timing');
  });

  it('keeps their line where they put it when the regroup nests it INSTEAD', () => {
    const { ydoc, own, basedOn, human } = typedInto();
    // The other half of the same hole: their line appears ONLY inside a
    // group. Deleting the copy must not delete the note — the original is
    // still in the doc, in their element, where they typed it.
    const regrouped = [
      '## Meeting notes',
      '',
      '- What the dialog gets wrong',
    ...
    mergeNotesSection(ydoc, regrouped, HEADING, { ownership: own, basedOn });

    expect(markdownOf(ydoc).split(HUMAN)).toHaveLength(2);
    expect(humanItems(ydoc)).toHaveLength(1);
    expect(humanItems(ydoc)[0]!.el).toBe(human);
  });

  it('splits the group rather than dropping it when THEIR line is the lead', () => {
    const { ydoc, own, basedOn, human } = typedInto();
    // The composer makes their note the head of a group of its own points.
    // Their line is not the agent's to re-lay, so the group is split: their
    // line stays where it is, and the points it gathered stay as bullets.
    const regrouped = [
      '## Meeting notes',
      '',
      `- ${HUMAN}`,
    ...
    const md = markdownOf(ydoc);
    expect(md.split(HUMAN)).toHaveLength(2);
    expect(humanItems(ydoc)[0]!.el).toBe(human);
    // Nothing the composer wrote was lost with the copy.
    for (const bullet of ['one', 'two', 'three', 'four']) {
      expect(md).toContain(`Agent bullet ${bullet}`);
    }
  });

  it('lets a subheading land above the bullets it groups, not below them', () => {
    const { ydoc, own, basedOn, human } = typedInto();
    // A heading between two items of one list is the shape the item-level
    // merge cannot express, and the relayout that used to express it is
    // refused as soon as one line in the section is a person's. So the
    // heading arrived BELOW every bullet — in exactly the meetings people
    // type in.
    const grouped = [
      '## Meeting notes',
      '',
    ...
    expect(merged.ok).toBe(true);
    const md = markdownOf(ydoc);
    // Each heading is above the bullets it groups, and below the ones it
    // does not — the order the model wrote, not the order the merge could
    // manage.
    expect(md.indexOf('### Cost')).toBeLessThan(md.indexOf('Agent bullet one'));
    expect(md.indexOf('Agent bullet two')).toBeLessThan(md.indexOf('### Scope'));
    expect(md.indexOf('### Scope')).toBeLessThan(md.indexOf('Agent bullet three'));
    // Their line is untouched, and untouched means the same element: this
    // path clears the agent's own bullets out of the list and never rebuilds
    // the section, so their marks and comment anchors ride through it.
    expect(md.split(HUMAN)).toHaveLength(2);
    expect(humanItems(ydoc)[0]!.el).toBe(human);
  });

  it('still refuses to restructure over a proposal pending in the section', () => {
    const { ydoc, own, basedOn } = typedInto();
    const proposed = suggestOps.suggestReplace(ydoc, {
      find: 'Agent bullet two',
      replace: 'Agent bullet two, corrected',
      author: { id: 'u-1', name: 'A Person', color: '#333' },
    ...
    // The same regroup as above — every bullet still listed, so nothing here
    // turns on the ordinary path's own deletes.
    const grouped = [
      '## Meeting notes',
      '',
      `- ${HUMAN}`,
    ...
    // Clearing the agent's bullets would have taken the proposal's element
    // with it. The heading lands wherever the ordinary merge can put it —
    // worse looking, and it cannot eat somebody's proposal.
    expect(suggestOps.listSuggestions(ydoc)).toHaveLength(1);
  });
});
