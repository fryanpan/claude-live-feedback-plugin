/**
 * Voice ACTIONS (§2.4 / §3.8): the first two executors.
 *
 ...
 * `taskStore.setAssignee`, the SAME choke points the REST routes use, so the
 * move is attributed to the speaker and lands in `events.jsonl` exactly as a
 * tapped one does. Anything outside the scoped verb set still goes to the
 * agent, unchanged.
 *
