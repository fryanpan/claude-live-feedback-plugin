...
The script writes `~/Library/LaunchAgents/com.fryanpan.claude-workspaces.plist`, bootstraps it into your user session, and waits for port 8788 to come up. Logs land at `~/Library/Logs/com.fryanpan.claude-workspaces.{out,err}.log`. The service runs as a per-user `LaunchAgent` — it starts at login (not boot) and stops at logout, which matches Bryan's Mac-mini-always-logged-in setup.
