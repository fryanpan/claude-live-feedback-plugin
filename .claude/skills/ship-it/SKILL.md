...
git diff --name-only <base-branch>...HEAD |
  grep -E 'packages/server/src/(server|bin|access-deps|identity-setup)\.ts|packages/server/src/middleware/|packages/server/src/auth/|packages/server/src/share/|packages/server/src/(webhooks|recall-webhook-auth|recall|fs-scan)\.ts'
```
