...
| `packages/server/src/tasks.ts` | 4458 | Split | A9 took the two seams the previous row named: the emit choke point and its audit trail to `task-event-bus.ts` (132 — `onEvent`/`emit`/`appendAudit`/`noteObservedWork`, behind a `TaskEventBusPersistence` contract so `emit`'s own order, audit-then-observed-work-then-listeners, is unchanged), and `persist`/`load` plus the four persistence-contract builders (`reviewItemPersistence`, `goalPersistence`, `agentPersistence`, `workspacePersistence`) to `task-persistence.ts` (447), behind one `TaskPersistenceHost` seam for the same reason `WorkspaceStore` / `GoalStore` / `AgentStore` already use one each. The pure nested-goal and attachment-runtime helpers both needed came out to `task-helpers.ts` (62) so neither new file has to import a value back from the file that imports it. What is left is the one responsibility the file's row has named since A2: task rows — create, transition, assign, note, review — now with no store-internal seam left to name. **S**, if the routes/verbs themselves ever separate from the `TaskStore` class they hang off. |
| `packages/server/src/rooms.ts` | 3527 | Split | A3 took document mutation out to `doc-edit-ops.ts`, threads to `doc-threads.ts` and the workspace surface to `rooms-workspaces.ts`; the file bindings this row named as the next seam are now `file-binding.ts` (see below), behind the `FileBindingHost` handle it asked for rather than a line range. What is left is two layers: the room lifecycle (`getOrCreate`, `hydrateDoc`, `evictIdleRooms`, `teardownRoom`, `flush`, the `.ydoc` persist and the doc index it writes) and the websocket fan-out (`wireEvents`, `broadcastToRoom`, `fireEvent`, presence). They are the next seam and a harder one than the bindings were: the fan-out reads `room.conns` and `room.seq` on every frame and is what a teardown has to close, so the handle it needs is the room object itself. **M** |
...
| File | Lines | Verdict | Reason / seam |
|---|---|---|---|
| `packages/workspaces-app/src/styles.css` | 2783 | Exception | The chrome every page shares, and only that: design tokens, the top bar, voice, the pointer pill, the editor's prose rules, the comment pill, the threads pane, both composers, the toast, the connection and stale-client banners, the read-only bar and the first-arrival identity prompt. B2 took the board's half into `hub.css` and the sign-in page's into `signin.css` (12042 -> 6545); this split took the review editor's own surfaces into `doc.css` (6545 -> 2783). Cutting the remainder further would split rules that three pages all download from each other and buy no page a byte, because every one of these three pages loads all of it. A rule goes under the banner naming the surface it paints (`grep -n '^/\* =' packages/workspaces-app/src/styles.css`); the test is whether more than one page can reach it, which is why `.thread-line` stayed here, why `.signin-bar` had to come back in B2, and why the suggested-edit marks and the `.lf-suggest-chip` default live here rather than beside the redline chrome they were authored next to: the board mounts the same `createEditor` over a task body, and `SuggestInsert` / `SuggestDelete` / `SuggestionChips` are base-schema, so a task description can carry a pending proposal. |
| `packages/workspaces-app/src/doc.css` | 3882 | Exception | The review editor's whole stylesheet — main layout, the review-set sidebar, the workspace file tree, the format bar, the reassign menu, code blocks and tables, the meeting record chrome and live zone, the over-doc sheet, the full-screen thread view and long-thread modal, the code-review and diff-review surfaces, the diff nav and the inline thread cards. Lifted out of `styles.css` in source order and loaded AFTER it, which is the half of this that was a decision: those rules sat interleaved through the base, so the split reorders 72,575 pairs of rules. None of them can decide a value in this order (same specificity, importance, property, element and pseudo-element box); twenty can in the other, which is why `index.html` links the two in the order it does. Same argument as `hub.css` for not cutting it per surface: it would trade labelled regions for an import graph between stylesheets, and the editor loads all of it on every visit. Two grouped selectors — the ≤1100px chip reveal and the print hide — are split across the two files rather than lifted whole, because `.lf-suggest-chip` is minted by a base extension and `.lf-del-chip` is not; each half keeps its declarations and its position verbatim, so neither chip's resolved value moves. |
...
| `packages/workspaces-app/src/hub/hub-app.ts` | 2166 | Exception | The hub's entry: it holds `main()`'s one `HubState` and owns the renders, the boot sequence and the address bar. B1 lifted the REST writes to `hub-actions.ts`, the review queue's controller to `hub-review-controller.ts` and the live path to `hub-live-wiring.ts`. A11 took the three pieces that did not need the whole closure: the static shell to `hub-shell.ts` (193 — it takes a document, a root, a name and a workspace id, and cannot reach `state` at all), the settings panel's three controls to `hub-settings-panel.ts` (181, twelve deps, the open flag as an `isOpen`/`setOpen` pair rather than the state object), and the Home walkthrough to `hub-walkthrough.ts` (271, eleven deps) — the walk's position, its queue and what answering does to both. None of the three is over the limit, so none has a row of its own. What is left is the render layer and the boot sequence, which share `state` and every `render*` closure by construction. The remaining seam a reader will reach for is the detail panel — `renderDetail` and the eight functions around it, ~600 lines — and it is the one A11 deliberately did not take: its deps run past thirty, which is the whole closure passed as an argument and called a seam. **S**, if the render closures ever stop sharing one `state`. |
