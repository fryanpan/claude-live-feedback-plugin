...
| File | Lines | Verdict | Reason / seam |
|---|---|---|---|
| `packages/workspaces-app/src/styles.css` | 2681 | Exception | The chrome every page shares, and only that: design tokens, the top bar, voice, the pointer pill, the editor's prose rules, the comment pill, the threads pane, both composers, the toast, the connection and stale-client banners, the read-only bar and the first-arrival identity prompt. B2 took the board's half into `hub.css` and the sign-in page's into `signin.css` (12042 -> 6545); this split took the review editor's own surfaces into `doc.css` (6545 -> 2681). Cutting the remainder further would split rules that three pages all download from each other and buy no page a byte, because every one of these three pages loads all of it. A rule goes under the banner naming the surface it paints (`grep -n '^/\* =' packages/workspaces-app/src/styles.css`); the test is whether more than one page can reach it, which is why `.thread-line` stayed here and `.signin-bar` had to come back in B2. |
| `packages/workspaces-app/src/doc.css` | 3942 | Exception | The review editor's whole stylesheet — main layout, the review-set sidebar, the workspace file tree, the format bar, the reassign menu, code blocks and tables, the meeting record chrome and live zone, the over-doc sheet, the full-screen thread view and long-thread modal, the code-review and diff-review surfaces, the diff nav and the inline thread cards. Lifted out of `styles.css` in source order and loaded AFTER it, which is the half of this that was a decision: those rules sat interleaved through the base, so the split reorders 72,575 pairs of rules. None of them can decide a value in this order (same specificity, importance, property, element and pseudo-element box); twenty can in the other, which is why `index.html` links the two in the order it does. Same argument as `hub.css` for not cutting it per surface: it would trade labelled regions for an import graph between stylesheets, and the editor loads all of it on every visit. |
