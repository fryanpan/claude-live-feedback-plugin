...
|---|---|---|---|
| `packages/server/src/server.ts` | 5358 | Split | A1 took five families out; A12 took the composition; A14 took the seam the A12 row named — the stall / ready-work wiring, out whole to `stall-wiring.ts` (935). What moved is one documented subsystem (docs/architecture/stall-detection.md): both per-board snapshots (`readyWorkSnapshot`, `stallSnapshot` and the `stallVerdict` under it), the held-item walk over comment-borne asks (`heldThreadReviewItems`), the lead-presence monitor, both nudger objects, the store subscription that feeds the ready clock, and the comment-queue bridge (`onDocRoomEvent` and the two helpers under it). `createServer` now composes it and arms the nudgers. The row's `hubBoardsForDoc` warning held and the fix was the one it suggested: that function, `backTargetFor` and `reviseCallFor` are all built below the wiring and all three are reached only from a request or an event, so they arrive on the context AS FUNCTIONS — nothing in `createServer` was reordered, and the review gate stayed where it is because it needs `resolveWorkspaceForDoc` and the projection. The next seam is the doc↔board membership map: `hubWorkspacesHolding` and the share-scope predicates over it (`shareWorkspacesOf`, `collabMemberOf`, `shareLinkMemberOf`, `redeemShareLink`), `hubBoardsForDoc` and its prebuilt-index twins (`boardIndexForListing`, `heldByIndexed`, `hubBoardsForDocIndexed`, `homeForDocIndexed`), `backTargetFor`, `defaultHubWorkspaceId`, the three filing verbs (`fileUnderHubWorkspace`, `unfileFromDefault`, `unlinkFromEveryHubWorkspace`) and the coverage readout that reports against exactly that set (`watchCoverageFor`, `queuedForLead`). Roughly 620 contiguous lines answering one question — which boards hold this doc, who may reach it through one, and does an agent's watch set actually cover them — and it is the seam this slice had to reach past, so taking it next turns two of the three thunks above back into values. **M** |
| `packages/server/src/stall-wiring.ts` | 935 | Exception | The stall / ready-work wiring, out of `createServer` in A14. One file because it is one reading: every member answers "is this board moving, and who has to be told", and the two snapshots exist only to be handed to the two nudgers that share the tick. Splitting ready from stall is the obvious cut and it is the wrong one — they share `parallelismCapView` / `capSummary` (the cap must mean the same number in both wakes), the same `buildQueue` priority order asked with `includeBlocked`, and the same `sse.agentsOn` addressing rule, and the comment-queue bridge is what makes a comment count as movement for the stall clock at all. Two files would put the ready wake's reading of the cap in one and the stall wake's in another with nothing asserting they agree, which is the drift the shared `evaluateStalls` / `evaluateReadyWork` pair exists to close. About 180 of these lines are the context interface and its doc — the price of the seam being explicit rather than a closure. **M**, if the comment-queue bridge (`onDocRoomEvent`, `queueCommentRows`, `markCommentRowsEmitted`, `commentAuthorIs` — roughly 140 lines) is worth its own file: it is the one member that is a delivery path rather than a reading of the board, and it came along because the stall loop is its only other reader. |
