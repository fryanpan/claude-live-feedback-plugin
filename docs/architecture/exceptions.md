...
| `packages/core/src/review-item.ts` | 697 | Split | The seam this row named has been taken: the twelve contract types are now `review-item-types.ts` (377, no row needed), which emits nothing, so `review-item-wire.ts` (333) reaches them without importing the verbs back. What is left is the payload lifecycle the earlier splits left behind — `applyReviewRevision`, `withdrawReview` / `reinstateReview`, `answerFromReply` and `pendingDeclaration`, `reviewItemState` and the small derivations under it — plus the facade that re-exports all three siblings under the names callers already import. The next seam is the quality-gate predicates, `isReviewItemHeld` / `isReviewItemGated` / `isReviewPayloadHeld` / `isReviewPayloadGated` / `reviewPayloadVersion` and the two `judgeReason*` renderers: ~120 lines that read a verdict nothing else here writes. **S** |
...
| 14 | `review-item-check.ts` + `review-item-wire.ts` + `review-item-types.ts` + `review-item.ts` (split in B5, contract lifted after) | 1769 | 18 | done |
