...
| File | Lines | Verdict | Reason / seam |
|---|---|---|---|
| `packages/workspaces-app/src/styles.css` | 2672 | Exception | The chrome every page shares, and only that: design tokens, the top bar, voice, the pointer pill, the editor's prose rules, the comment pill, the threads pane, both composers, the toast, the connection and stale-client banners, the read-only bar and the first-arrival identity prompt. B2 took the board's half into `hub.css` and the sign-in page's into `signin.css` (12042 -> 6545); this split took the review editor's own surfaces into `doc.css` (6545 -> 2672). Cutting the remainder further would split rules that three pages all download from each other and buy no page a byte, because every one of these three pages loads all of it. A rule goes under the banner naming the surface it paints (`grep -n '^/\* =' packages/workspaces-app/src/styles.css`); the test is whether more than one page can reach it, which is why `.thread-line` stayed here and `.signin-bar` had to come back in B2. |
