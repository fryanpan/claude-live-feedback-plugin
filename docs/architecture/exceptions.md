...
| File | Lines | Verdict | Reason / seam |
|---|---|---|---|
| `packages/markdown-app/src/styles.css` | 6713 | Split | The board's half came out into `hub.css` in B2, which is what took this file from 12042. What is left is the review editor's own surfaces plus the chrome every page shares — design tokens, top bar, toast, connection banner, the identity prompt — and the sign-in block at the end, which B2's second commit takes. After that this row's remainder is one page's stylesheet plus a shared base that no page can drop; splitting THAT needs a measured list of which blocks each page reaches, which the B2 PR body records and no commit has acted on. |
| `packages/markdown-app/src/hub.css` | 5347 | Exception | The board's whole stylesheet, lifted out of `styles.css` in B2 in source order so no rule changed the cascade it lands in. It is one page, and the 28 `##### HUB · …` sub-banners are the split: a rule goes under the banner naming the surface it paints (`grep -n '##### HUB' packages/markdown-app/src/hub.css`). Cutting it into files per surface would trade a labelled region for an import graph between stylesheets, and the board loads all of it on every visit either way. |
