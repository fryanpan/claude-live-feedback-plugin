...
| `packages/workspaces-app/src/review-chrome.ts` | 803 | Split | B3 took the two writing surfaces to `doc/review-composer.ts`, the panel resize handle and thread-range clicks to `doc/chrome-panels.ts`, and the shared DOM helpers to `doc/chrome-dom.ts`. The seam this row named — the thread projection — is now `doc/thread-projection.ts` (260): `collect`, `resolveRange`, `refreshDecorations`, `lineLabel` and the three pieces of state only they touch (the active thread id, the pending-summary expiry timer, the seen tracker's in-place dot removal), with the ydoc, the surface and the tracker passed in and one `onPendingExpiry` callback back. The same pass took the per-device view preferences to `doc/view-prefs.ts` (122, re-exported from here because existing tests import them from this module) and the five server writes a card can make to `doc/thread-actions.ts` (156). What remains is `mountReviewChrome`'s wiring: the drawer and its tabs, the panel and the wide modal and the routing between them (`engageThread`, `maybeOpenModal`, `revealThread`), the mobile mount, the composer handoff, the doc label, the ydoc observer, the hotkeys and `destroy`. The next clean cut is the doc-title label (`renderDocLabel`, `docLabel`, `mobileLabel`, ~70 lines) — the one remaining piece that reads nothing the mount builds. The engagement router is the seam a reader will reach for first and is NOT it: it reads four of the five handles the mount constructs (`threadsPanel`, `threadModal`, `mobile`, `surface`), so lifting it would mean passing the mount back in. **M** |
...
| 11 | `review-chrome.ts` + `doc/thread-projection.ts` + `doc/thread-actions.ts` + `doc/view-prefs.ts` (was `review-chrome.ts`, split in B3 and again on 2026-09-03) | 1089 | 24 | M |
