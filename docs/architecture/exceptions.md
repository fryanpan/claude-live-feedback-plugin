...
A10 split `rooms.ts` (4801 → 3490) and added one row for `file-binding.ts`
(1652), the seam the `rooms.ts` row had named. A split that lifts an oversized
subject out of an oversized file adds to this list rather than shortening it —
what changes is that each row now describes one thing.
...
| `packages/server/src/tasks.ts` | 4284 | Split | A9 took the two seams the previous row named: the emit choke point and its audit trail to `task-event-bus.ts` (132 — `onEvent`/`emit`/`appendAudit`/`noteObservedWork`, behind a `TaskEventBusPersistence` contract so `emit`'s own order, audit-then-observed-work-then-listeners, is unchanged), and `persist`/`load` plus the four persistence-contract builders (`reviewItemPersistence`, `goalPersistence`, `agentPersistence`, `workspacePersistence`) to `task-persistence.ts` (447), behind one `TaskPersistenceHost` seam for the same reason `WorkspaceStore` / `GoalStore` / `AgentStore` already use one each. The pure nested-goal and attachment-runtime helpers both needed came out to `task-helpers.ts` (62) so neither new file has to import a value back from the file that imports it. What is left is the one responsibility the file's row has named since A2: task rows — create, transition, assign, note, review — now with no store-internal seam left to name. **S**, if the routes/verbs themselves ever separate from the `TaskStore` class they hang off. |
| `packages/server/src/rooms.ts` | 3490 | Split | A3 took document mutation out to `doc-edit-ops.ts`, threads to `doc-threads.ts` and the workspace surface to `rooms-workspaces.ts`; the file bindings this row named as the next seam are now `file-binding.ts` (see below), behind the `FileBindingHost` handle it asked for rather than a line range. What is left is two layers: the room lifecycle (`getOrCreate`, `hydrateDoc`, `evictIdleRooms`, `teardownRoom`, `flush`, the `.ydoc` persist and the doc index it writes) and the websocket fan-out (`wireEvents`, `broadcastToRoom`, `fireEvent`, presence). They are the next seam and a harder one than the bindings were: the fan-out reads `room.conns` and `room.seq` on every frame and is what a teardown has to close, so the handle it needs is the room object itself. **M** |
