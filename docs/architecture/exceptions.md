...
**154 files** over 500 lines: 59 source and 95 test. **34 Split**,
**120 Exception**.
...
| `packages/server/src/tasks.ts` | 6560 | Split | `class TaskStore` still runs over three responsibilities; `review-items/` and now `workspace-store.ts` are the in-file precedent that extraction works. A2 took the workspace registry out. Still queued: agent presence and delivery queues (`attachAgent`, `heartbeat`, `leadSeatHealth`, `queueComment`) → `task-agents.ts`; goal-list machinery (`setGoalList`, `renameGoal`, `reorderGoals`, `setTaskGoal`) → `task-goals.ts`. **L** |
...
| `packages/markdown-app/src/hub/hub-model.ts` | 3645 | Split | Three unrelated pure models share one file: board layout and effort (`boardSections`, `boardEffort`, `dropTarget`) → `hub-board-model.ts` ~1200; the review and walkthrough queue (`reviewQueue`, `decisionQueue`, `advanceWalk`) → `hub-review-model.ts` ~1100; presence, release drift and the activity trail (`presenceChips`, `pluginDriftNotice`, `describeEvent`) → `hub-presence-model.ts` ~1200. All DOM-free and already unit-tested, so the move is mechanical. **M** |
| `packages/markdown-app/src/hub/hub-app.ts` | 3594 | Split | `main()` is a ~3000-line closure over one `HubState`. Lift the REST mutation verbs (`transitionTask`, `assignTask`, `placeTask`, `addGoal`) into `hub-actions.ts` ~600 behind an explicit deps object; the review controller (`openReviewItem`, `startWalkthrough`, `answerDecision`) into `hub-review-controller.ts` ~700; the boot and stream tail (ydoc observers, SSE listeners, catch-up) into `hub-live-wiring.ts` ~600. **L** — the closure capture is the work. |
| `packages/markdown-app/src/hub/hub-render.ts` | 2707 | Split | The renderers already cluster by region: task detail (`detailFields`, `effortFields`, `renderRelatedLinks`) → `hub-detail-render.ts` ~900; the comment stream (`flattenComments`, `discussionStream`, `commentRow`) → `hub-discussion-render.ts` ~650; the review panel (`panelReviewQueue`, `panelAnswerRequest`, `reviewItemRow`) → `hub-review-render.ts` ~500, leaving shell chrome. **M** |
...
| 7 | `packages/server/src/tasks.ts` | 6560 | 87 | L |
