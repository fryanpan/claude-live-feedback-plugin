...
|---|---|---|---|
| `packages/server/src/server.ts` | 4841 | Split | A1 took five families out; A12 took the composition; A14 took the stall / ready-work wiring out whole to `stall-wiring.ts` (938). A15 took the seam the A14 row named — the doc↔board membership map, out whole to `board-membership.ts` (803). What moved is one question asked from four directions: which boards hold an id (`hubWorkspacesHolding` and the prebuilt-index twins `boardIndexForListing` / `heldByIndexed` / `hubBoardsForDocIndexed` / `homeForDocIndexed`), which boards a DOC reaches (`hubBoardsForDoc`, `backTargetFor`), who may reach it through one (`shareWorkspacesOf`, `collabMemberOf`, `shareLinkMemberOf`, `redeemShareLink`), and whether an agent's watch set covers them (`watchCoverageFor`, `queuedForLead`) — plus the three verbs that WRITE the links all of that reads (`fileUnderHubWorkspace`, `unfileFromDefault`, `unlinkFromEveryHubWorkspace`), `defaultHubWorkspaceId` and the four `Coverage*` / `WatchCoverage` types, re-exported from here because that is the module name they have always been importable under. The A14 row's prediction held exactly: because nothing the map reads is built later than the stores, `createServer` composes it ABOVE the stall wiring and two of that wiring's three thunks (`hubBoardsForDoc`, `backTargetFor`) are now plain values. `reviseCallFor` stays a function — the review gate is still built below, for the reason it says where it is built. Nothing else was reordered. The next seam is the Home pane: `homeBriefs`, `homeBriefInflight`, `reviewItemsFor`, `homeQueueTotal`, `homeBriefInput`, `generateHomeBriefFor` and `homePayload` are ~225 contiguous lines answering one question — what one person sees on one board's Home tab, and what the brief may claim about it. `reviewItemsFor` is the one that makes it a family rather than a pile: the queue it builds is both the rendered list and the denominator `homeQueueTotal` prints, so the count cannot drift from the list. It needs the task store, the projection's `taskBodyDocId`, `rooms.listThreads` and the summarizer seam, all built above it. **M** |
