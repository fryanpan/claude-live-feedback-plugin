...
| **Domain (pure)** | `task-owner.ts`, `task-fields.ts`, `task-row.ts`, `decision-shape.ts`, `safe-path.ts`, `diff-groups.ts`, `pause-ticker.ts`, `meeting-capture-guards.ts`, `meeting-capture-prompt.ts`, `notes-section.ts`, `voice-prompt.ts`, `voice-action.ts`, `voice-resolve.ts`, `voice-status.ts`, `ask-detection.ts`, and most of what the split extracts | core | Functions over values. No clock, no filesystem, no socket, unless passed in. |
