...
| **Services / stores** | `rooms.ts`, `tasks.ts` and the board stores it delegates to (`workspace-store.ts`, `task-agents.ts`, `task-goals.ts`), `review-items/**`, `share/**`, `auth/**`, the `meeting-*` family, `sse.ts`, `activity.ts` | domain, infra, core | Owns durable state and orchestration. Must never import a route. |
| **Domain (pure)** | `task-owner.ts`, `task-fields.ts`, `task-row.ts`, `decision-shape.ts`, `safe-path.ts`, `diff-groups.ts`, and most of what the split extracts | core | Functions over values. No clock, no filesystem, no socket, unless passed in. |
