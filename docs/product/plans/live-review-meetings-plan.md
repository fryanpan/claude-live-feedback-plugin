...
following a collaborator. First real use: **an external partner reviewer, today/early
tomorrow.** Motivating session: Bryan + a reviewer reviewing the ADFA Quick
...
| P0 | Public URL for the external reviewer (Cloudflare share) | Infra + Bryan steps | blocked on TLS decision + credentials |
...
## P0 — Public URL for the external reviewer (Cloudflare, un-tabled)

the external reviewer has no Tailscale access, so the meeting needs the share flow from
...
3. Share with `--allow-domain @<greenlue-domain>` (domain TBD from
   Bryan/the external reviewer). Access = per-share Cloudflare Access app, email-OTP
...
- Sidebar shows presence dots per doc ("the external reviewer is in foo.md").
...
## Appendix: worked example — Bryan + a reviewer, ADFA Quick Build review
...
didn't replace android.jar (an engineer + Claude research notes); known-
...
an engineer to the low-spec ticket (the ExtraHeapMemory-style flag changed
...
**Signal worth keeping:** a reviewer called out the *proxying* and *deliberate
