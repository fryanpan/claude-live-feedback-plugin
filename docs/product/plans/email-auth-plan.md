...
- Identity is real from the first comment: it carries their chosen name ("Bold Lemur"), backed by a stable internal id derived from the email — the id is plumbing, like a row key, and never appears anywhere a person reads. The address is folded to lowercase as a whole before deriving (not domain-only — every real provider treats the local part case-insensitively despite the RFC, and whole-address folding is what keeps `Bryan@`/`bryan@` from becoming two people; see `normalizeEmail` in `packages/core/src/identity.ts`). The guest- path retires for new shares.
...
4. **App session**. cw_session: HMAC-signed, HttpOnly, SameSite=Lax, Secure derived from the real client scheme. No expiry until revoked (Bryan, 2026-08-28) — which makes server-side revocation load-bearing: the cookie carries a session id checked against the roster, so logout or a future "sign out everywhere" actually ends it. Minted only after email verification, by either path.
