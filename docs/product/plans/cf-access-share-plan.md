...
| ------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Markdown doc                                | Agent: `create_review_doc(docId, path)` → reviewer opens `mac-mini.<private-network>:8788/review/<docId>` over Tailscale/LAN | Same `create_review_doc`. Plus `bun share doc <docId>` → reviewer opens `share-<slug>.tunnel.fryanpan.com/review/<docId>` over the public internet, gated by Access. |
...
1. `bun share <docId> --allow-domain @appdevforall.org` brings up a public URL of the form `https://share-<slug>.tunnel.fryanpan.com/review/<docId>` within 10 seconds.
2. Visiting that URL from an unauthenticated browser redirects to a Cloudflare Access login page (email-OTP by default).
3. Login with an `@appdevforall.org` email succeeds and lands the reviewer on the live-feedback editor.
...
User can override with `--name <slug>`: `bun share <doc> --name appdevforall-mockup` → `share-appdevforall-mockup.tunnel.fryanpan.com`.
...
- **Integration (manual, one-off):** Bryan runs `bun share <real-docId> --allow-domain @appdevforall.org`, opens the URL in incognito with a non-`@appdevforall.org` email (expect deny), then with an `@appdevforall.org` email (expect allow), comments, resolves, Ctrl+C, confirms CF dashboard shows app deleted.
